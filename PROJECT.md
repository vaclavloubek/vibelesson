...
Aktualizováno: 2026-09-24 — interní verze **0.9.120**: Live Control Worker **0.8.16**. Když aplikace přejde na blok s časovačem (start hodiny i navigace), dostane časovač v záložní cestě plnou délku místo `null`. `null` se přes `reconcile_live_control_snapshot` zapsal do DB jako `timer_remaining_seconds = NULL` a studenti i učitel viděli 0:00 (živá hodina 24. 9., blok 8). Version ID se doplní po nasazení. Veřejně zobrazovaná verze na dashboardu zůstává 0.9.30.
...
- nasazení přes Wrangler (`npx wrangler deploy` v `cloudflare/live-control`) až po merge a jen ve chvíli, kdy v Neonu není žádná hodina `live`; Version ID a ověření `/health` se doplní po nasazení (předchozí Version ID `f71119be-808c-43fa-bffc-894bd762ab9c`);
