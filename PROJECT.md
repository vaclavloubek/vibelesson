...
- **Otevřené body:** (1) `reconcile_live_control_snapshot` na Neonu padá (`text = jsonb`), záložní dosynchronizace z Cloudflare proto nefunguje; běžné ovládání hodiny funguje. (2) Za provozu neověřeno: školní administrace, Stripe webhook (první obnova předplatného 18.–19. 10.), registrace nového uživatele. (3) Ostatní 3 účty si musí nastavit heslo přes „Zapomenuté heslo“. (4) Ojedinělé `P0001` u `/api/ai-quota` sledovat. Interní verze zůstává **0.9.92** (infrastrukturní přechod bez změny produktu).
