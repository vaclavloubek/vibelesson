...
- Terms rollout proběhl bezpečně ve třech DB krocích: **20260921071533_update_terms_1_1_legal_007**, okamžitý kompatibilitní guard **20260921072059_restore_terms_1_0_rollout_guard** a finální backward-compatible vrstva **20260921072159_add_versioned_terms_acceptance_rpcs**. Produkční signup trigger umí přesně auditovat v1 i v2 podle skutečně odeslaného key a nový web používá service-only RPC s explicitním `TERMS_ACCEPTANCE_KEY`; historické acceptance eventy se nemění ani nemažou;
- staré v1 RPC zůstává během rollout kompatibility pouze pro předchozí build; nový 0.9.86 používá verzované RPC `has_terms_acceptance_for_service` / `record_terms_reconsent_for_service` s explicitním key. Všechny nové i staré re-consent RPC zůstávají pouze pro **service_role** a Security Advisor nepřidal nový anon/authenticated privileged-function warning;
