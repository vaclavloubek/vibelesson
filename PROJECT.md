...
Aktualizováno: 2026-09-20 — interní verze **0.9.66** sjednocuje ukotvení přihlašovacího popoveru napříč aplikací. Auth dialog už není absolutně navázaný na lokální layout rodiče; používá viewport-safe fixed positioning odvozené z reálné pozice triggeru, automaticky se překlápí nad tlačítko při nedostatku místa pod ním, drží bezpečné okraje viewportu a přepočítává se při scrollu, resize i změně výšky obsahu. Tím se opravuje školní objednávka a zároveň se stejná ochrana vztahuje na landing, Pricing, lesson workspace, školní pozvánku a sdílenou lekci. Veřejně zobrazovaná verze na dashboardu zůstává 0.9.30.
...
### Viewport-safe přihlašovací popover 0.9.66 — 2026-09-20

- společný `AuthControls` nyní kotví přihlašovací dialog vůči viewportu podle skutečné pozice tlačítka, ne vůči nejbližšímu layoutovému rodiči;
- pokud se dialog nevejde pod trigger, překlápí se nad něj; pokud se nevejde ani tam, zůstane uvnitř viewportu a získá vlastní vertikální scroll;
- pozice se přepočítává při scrollu, resize, změně visual viewportu a při změně výšky formuláře přes `ResizeObserver`;
- odstraněn byl speciální mobile-only positioning override na landing page, takže landing, Pricing, lesson workspace, School, School invite a shared lesson používají jednu společnou logiku;
- regresní ochrana `verify-header-account-menu.mjs` nyní kontroluje i školní a share scénáře a zakazuje znovuzavedení lokálního positioning override;
...
- **0.9.66 interní UX — viewport-safe přihlašovací popover** — společné přihlášení se kotví podle triggeru přímo ve viewportu, umí se překlápět nad tlačítko, respektuje hrany obrazovky a průběžně se přepočítává při scrollu/resize i změně obsahu. Oprava pokrývá School, School invite, shared lesson, landing, Pricing a lesson workspace bez lokálních positioning výjimek. Veřejně zobrazovaná verze zůstává 0.9.30.
