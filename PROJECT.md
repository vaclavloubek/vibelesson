...
- **[LEGAL-004 — RESOLVED 0.9.82] Individuální elektronický nákup neposkytoval neměnnou kopii smlouvy a VOP v textové podobě.** Opraveno: server před Stripe Checkout vytvoří append-only snapshot tarifu, ceny, období, VOP a spotřebitelských informací, propíše jeho UUID do Stripe metadata a sváže jej s konkrétní Checkout Session. Po aktivaci zákazník obdrží smluvní shrnutí a dvě HTML přílohy — přesný snapshot smlouvy + přijatých VOP a vzorový formulář pro odstoupení. Obsah je chráněn SHA-256 a při doručení se znovu ověřuje. Privacy Notice 1.3 popisuje důkazní záznam i jeho retenci.
