...
- **předpoklad:** Stripe event destination musí mít zapnutý `invoice.upcoming` (dosud posílá subscription lifecycle + `invoice.payment_failed` + `invoice.paid`) a Stripe Billing nastavení „upcoming renewal events“ určuje, kolik dní před obnovou event přijde. Bez toho se kód nikdy nespustí;
...
- Resend automatizace **Syllonaut · Win-back after cancellation** (`01a0cf6b-88bf-7396-9a84-ff7f66534457`) čeká na tento event, po 14 dnech znovu ověří opt-in a `contact.plan == free` a podle `ui_locale` a `event.plan_code` pošle jednu ze 4 šablon (CS/EN × Teacher/Teacher Pro). **Je zatím disabled** a zapne se až po nasazení této verze;
