...
- `manual_entitlement_overrides` zachovává explicitní beta/admin výjimky nad základním plánem; od 0.9.116 může přes `plan_code` + `expires_at` časově omezeně přidělit i celý individuální tarif (např. týdenní zkušební Teacher Pro), vypršení řeší hodinový cron;
