...
- produkčně jsou aktivní (stav 2026-09-23) tyto automations: **Syllonaut · Onboarding**, **Syllonaut · First live → Paid**, **Syllonaut · Free quota near limit**, **Syllonaut · Free quota reached**, **Syllonaut · Paid onboarding**, **Syllonaut · Win-back after cancellation**, **Syllonaut · Renewal tips before billing period ends**, **Syllonaut · Organization owner activated** a **Syllonaut · Organization member joined** (organizační od 2026-09-24);
...
- od 0.9.142 navíc `syllonaut.grading_quota.reached` (payload `used`, `limit`, `reset_date`, `quota_scope`, `plan_code`), jednou za okno kvóty, když dojdou návrhy hodnocení od AI; u organizace jde vlastníkovi. Resend automatizace **„Grading quota reached“** zatím neexistuje, založí ji Cowork po nasazení (detail v sekci 0.9.142);
- produkční `RESEND_API_KEY` musí mít **Full access** (klíč „Syllonaut app“ od 2026-09-24); Preview používá samostatný klíč „Syllonaut preview“ jen pro odesílání, aby testovací nasazení nezakládala kontakty ani nespouštěla automatizace; klíč jen pro odesílání vrací u Contacts a Events API `401 restricted_api_key` a lifecycle tiše nefunguje;
...
- dočasné review/test automations (`TEST · …`) jsou po testu **disabled**; všech 9 produkčních automations uvedených výše je **enabled**.
