...
Aktualizováno: 2026-09-19 pro interní verzi 0.9.39 nad veřejným releasem 0.9.30 — doladěna adresa v join kartě Prezentačního režimu. `www.syllonaut.com/join` se již nesmí zalomit uprostřed cesty; používá vlastní menší responzivní `clamp()` velikost, lehce utažený tracking a `white-space: nowrap`. Oprava zachovává layout z 0.9.38 pro QR i kód hodiny a přidává regresní kontrolu proti návratu `overflow-wrap: anywhere`. Veřejně zobrazovaná verze na dashboardu zůstává 0.9.30.
