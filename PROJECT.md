...
- **Právní audit:** vyřešeno **LEGAL-001 až LEGAL-018 a LEGAL-020 až LEGAL-024** (23. 9. LEGAL-013 až 018, 020 a 021: #293, #294, #296, #298, #300, #302, #304; 24. 9. LEGAL-022; 25. 9. LEGAL-023; 26. 9. LEGAL-024). Aktuální **VOP 1.13** (`2026-09-26-v14`, účinné od 26. 9. 2026; článek 5a o balíčcích návrhů hodnocení od AI, článek 6 doplněn o ukončení předplatného při plném vrácení platby; souhlasy v4–v13 zůstávají dostatečné), **Privacy Notice 1.11** (26. 9. 2026, localStorage s e-mailem čekajícím na potvrzení kódem; 1.10 evidence potvrzení upozornění Free na jedno živé použití lekce), **DPA 1.4** (`2026-09-25-dpa-v5`), Neon migrace **0011** (evidence reklamací) a **0012** (body z AI až po potvrzení učitelem) aplikované v produkci. Otevřené: **LEGAL-019** (DPH/OSS — daňový poradce); **LEGAL-021** (AI Act) vyřešen v 0.9.105, posouzení potvrdit právníkem a do 2. 12. 2026 vyřešit označení textu generovaného AI (čl. 50 odst. 2).
- **Otevřené provozní body:** Balíčky návrhů hodnocení od AI jsou od 25. 9. v provozu (testovací LIVE nákup, grant, refund i banner v hodině ověřené, blok v Resend šabloně doplněný); souběh `invoice.paid` s vytvořením předplatného vrací jednorázově 500 (Stripe opakuje, viz záznam 25. 9. o testovacím nákupu); Nápověda Syllonautu: hlídat útratu klíče AI Gateway `syllonaut-help` (limit $5/měsíc) a `help_assistant_requests` (stav, `cost_usd`, `feedback`); ruční úpravu aktivity (0.9.124) ověřit s Free účtem (s admin účtem ověřeno); hlídat limit AI Gateway, protože API klíč produkce má vlastní limit **$20/měsíc** (24. 9. vyčerpaný limit $10 zastavil AI hodnocení); za provozu neověřené školní administrace a Stripe webhook (první obnova 18.–19. 10.); registrace nového uživatele i spuštění Onboardingu po ověření kódem ověřeny 26. 9.; ostatní 3 účty si musí nastavit heslo; sledovat ojedinělé `P0001` u `/api/ai-quota`; skutečné podání reklamace a e-maily reklamací nebyly zkoušeny (trvalý append-only záznam); přímý Neon re-consent zápis (účty se souhlasem starším než v4) nebyl spuštěn proti DB; limit důvěryhodných zařízení je zatím jen v Ceníku, ne ve VOP; po přijetí českého § 1830a OZ znovu porovnat online odstoupení.
...
Aktualizováno: 2026-09-26 — **podpora jednoho Free účtu** (provozní zásah v produkční databázi, bez změny verze a kódu). (1) Učitelka omylem otevřela hodinu; zůstala v lobby bez jediného připojeného studenta. Protože se Free lekce zamyká až prvním připojeným studentem (`lesson_reuse_usage` / `lesson_live_usage`), lekce spotřebovaná nebyla a nebylo co resetovat. Hodinu v lobby jsem ukončil stejným zápisem jako tlačítko „Ukončit hodinu“ (`update public.sessions set status = 'ended', ended_at = now(), timer_status = 'idle', timer_started_at = null` s podmínkou `status = 'lobby'` a vlastníka; zprávu Workeru jsem neposílal, nikdo nebyl připojený). Na přání vlastníka hodinu nemažu (učitelka ji chce spustit znovu); prázdná ukončená hodina se jí může ukázat ve Výsledcích. (2) Na pokyn vlastníka má účet zkušební **Teacher Pro na 3 dny** přes `manual_entitlement_overrides` (`plan_code = 'teacher_pro'`, `expires_at` 2026-09-29 07:29 UTC) a `private.recompute_current_profile_entitlements`; ověřeno `profiles.active_plan_code = 'teacher_pro'` a `lesson_reuse_enabled = true`. Po vypršení výjimku smaže hodinový cron a účet se vrátí na Free.

Aktualizováno: 2026-09-26 — interní verze **0.9.176** (menší úprava, veřejná verze zůstává **0.9.170**): **formulář pro kód z e-mailu přežije obnovení i zavření stránky** (požadavek vlastníka po registraci z 25. 9., kdy se stránka po registraci znovu načetla a formulář zmizel). `components/AuthControls.tsx` si po registraci a po přihlášení účtu s nepotvrzeným e-mailem uloží do `localStorage` (`syllonaut-pending-email-verification-v1`) adresu čekající na kód a čas uložení. Po načtení libovolné stránky s přihlašovacím oknem, když nikdo není přihlášený, se okno samo otevře na „Potvrďte e-mail“ s touto adresou. Záznam platí nejvýše 24 hodin a smaže se po potvrzení e-mailu, po přihlášení, tlačítkem „Zpět k přihlášení“ nebo když je uživatel už přihlášený. Nedostupné úložiště (soukromý režim) nic nerozbije, formulář se jen znovu neotevře. **Privacy Notice 1.11** (`/gdpr`, účinná od 26. 9. 2026, rozhodnutí vlastníka): v tabulce sekce 5 nový řádek pro toto úložiště (účel, nezbytné, nejvýše 24 hodin). `verify-privacy` hlídá verzi 1.11 a nově, že každý klíč úložiště z `AuthControls` je v tabulce uvedený. `verify-neon-auth-email` hlídá uložení při registraci a nepotvrzeném přihlášení, obnovení formuláře a smazání po potvrzení. Ověřeno v `next dev` (stub server akcí, mobil 375 px): registrace → formulář pro kód → nová záložka, stará zavřená, stránka `/new` → okno se samo otevřelo na „Potvrďte e-mail“ se správnou adresou; totéž na `/cs`; po zadání kódu „E-mail je potvrzený“, záznam smazán a po dalším načtení se okno neotevře; záznam starší než 24 h se ignoruje a smaže. Kontroly: `npx tsc --noEmit`, `npm run check`.

Aktualizováno: 2026-09-26 — interní verze **0.9.175** (oprava, veřejná verze zůstává **0.9.170**): **přihlášení účtu s nepotvrzeným e-mailem znovu nabídne zadání kódu.** `signInWithNeonForApp` od 0.9.104 hledal chybový kód `EMAIL_NOT_VERIFIED`. Serverové SDK `@neondatabase/auth` (0.5.0-beta) ale kódy Better Auth normalizuje a pro nepotvrzený e-mail vrací `email_not_confirmed` (status 422). Pokus o přihlášení proto nikdy nepřepnul na zadání kódu: uživatel viděl „Přihlášení se nepodařilo. Zkontroluj e-mail a heslo.“, zatímco Neon Auth (odesílání při přihlášení) na pozadí poslal nový kód. Tak skončila registrace z 25. 9.: podle produkčních logů se stránka po registraci znovu načetla (formulář pro kód tím zmizel), pak přišly dva pokusy o přihlášení 5 s po sobě a každý poslal kód. Uživatelka hlásí, že kód přišel, ale nebylo kam ho zadat. Oprava záznamu o ověření Onboardingu výše: účet registraci nedokončil kvůli této chybě, ne vlastní vinou. Nově akce přijme `email_not_confirmed` i `EMAIL_NOT_VERIFIED` a vrátí `needsVerification`, takže se zobrazí formulář „Potvrďte e-mail“ se zprávou „E-mail ještě není potvrzený. Poslali jsme ti nový kód.“ Ověřeno: skutečná akce se skutečným SDK a falešnou odpovědí Neon Auth `403 EMAIL_NOT_VERIFIED` (Node + `registerHooks`) vrací před opravou jen `{ error: 'Email verification required' }`, po opravě `needsVerification: true`. V `next dev` (stub akcí, mobil 375 px) přihlášení zobrazí formulář pro kód a po registraci se formulář pro kód zobrazí i na počítači. `verify-neon-auth-email` nově čte kód pro nepotvrzený e-mail z nainstalovaného SDK místo pevného textu; na původním kódu selže. **Zůstává:** formulář pro kód po registraci drží jen stav stránky; po jejím znovunačtení (např. návrat z pošty na telefonu) vede cesta přes přihlášení, které nyní funguje.

Aktualizováno: 2026-09-26 — **produkční nasazení 0.9.174 a migrace `0025`** (bez změny verze). #421 (`60872fa`) je v produkci: GitHub deployment `6675706848` Production `success`. **Migrace `0025_scoreboard_skips_unrevealed_active_quiz.sql` je aplikovaná v produkci** (po souhlasu vlastníka, Neon konektor, jeden příkaz). Těsně předtím měla produkční funkce md5 `809ca2e910f505b46b10bd467a012abe` (beze změny od 0012), po aplikaci má `get_student_public_scoreboard` md5 `184070ff414deb0ce8dad4632383b04a`, shodné s ověřenou dočasnou větví. Práva (`authenticated` + vlastník `neondb_owner`), `SECURITY DEFINER` a `search_path=public, pg_temp` beze změny, funkce nemá přetížení. Dočasná Neon větev smazána. Schema cache Data API se neobnovovala (funkce se volá přímým SQL). Neověřeno: chování v živé hodině se studentem (aktivní kvíz se zveřejněným pořadím).

Aktualizováno: 2026-09-26 — **ověření Onboardingu po registraci na Neonu, ruční aktivace účtu a úklid testovacího účtu** (bez změny verze, bez změny kódu). Podezření, že potvrzení registrace kódem (`verifyNeonEmailForApp`) neposílá `syllonaut.onboarding.started`, se nepotvrdilo: volání je v kódu od 0.9.115 a `verify-marketing-lifecycle` ho hlídá. Automatizace **Syllonaut · Onboarding** neměla od 24. 9. nový běh, protože účet registrovaný 25. 9. registraci nedokončil: dostal 3 doručené e-maily „Dokončete registraci do Syllonautu“ (17:18, 17:21:25 a 17:21:30 UTC), žádný kód nezadal a `private.marketing_lifecycle_failures` je prázdná. Jiný ověřený účet se souhlasem bez Onboardingu není. **Produkční test vlastníka** (nová registrace 26. 9. 06:32 UTC): přišel jediný e-mail s kódem a 40 s po ověření odešlo uvítání Onboardingu. Tím je Neon cesta ověřená i za provozu. Neon Auth při registraci vždy založí v `neon_auth.verification` jeden kód navíc (i s `send_verification_email_on_sign_up = false`) a aplikace ho vlastním kódem hned nahradí. V produkci ho nikdo neposílá; dva e-maily s kódem chodí jen na Preview, kde je webhook e-mailů vypnutý. Počet řádků v `neon_auth.verification` tedy neodpovídá počtu e-mailů. **Ruční aktivace:** na pokyn vlastníka dostal účet z 25. 9. `emailVerified = true` (jeden chráněný `UPDATE` v `neon_auth."user"`, trigger přenesl stav do `app_identity.users`). Pak byl v Resendu založen kontakt (`opt_in`, `cs`, `free`) a poslán `onboarding.started`: uvítání odešlo a běh čeká na první lekci. Uživatelku informuje vlastník. **Úklid:** testovací účet z 26. 9. smazán podle `ACCOUNT_DELETION_RUNBOOK.md` (Resend kontakt, uživatel Neon Auth, `app_identity.users` s kaskádou profilu a souhlasu; záznam o přijetí VOP ponechán). Jeho běh Onboardingu v Resendu zůstal ve stavu „running“, ale bez kontaktu neprojde podmínkou souhlasu a nic dalšího neodešle.

Aktualizováno: 2026-09-26 — interní verze **0.9.174** (oprava, veřejná verze zůstává **0.9.170**): **průběžné pořadí už neprozradí správnost odpovědi na aktivní kvíz.** Při zveřejněném pořadí počítala DB funkce `get_student_public_scoreboard` každý kvíz až po aktivní blok bez ohledu na `revealed_block_ids`. Student tak po odpovědi na aktivní kvíz, jehož výsledky ještě nebyly zveřejněné, viděl asi za 3 s (push z Workeru → sloučený `refresh()`) změnu svého skóre, a protože odpověď lze měnit, dokud učitel nezveřejní výsledky nebo nepřejde dál, mohl možnosti zkoušet, dokud skóre nestouplo. Stejně se měnilo pořadí na projektoru. Nově se během živé hodiny **aktivní kvíz nezapočítá do skóre, `maxPoints` ani pořadí, dokud nejsou zveřejněné jeho výsledky** (`reveal_results`). Po zveřejnění výsledků, po přechodu učitele na další blok (odpověď už nejde změnit) a po skončení hodiny se počítá jako dřív. (1) Neon migrace **`0025_scoreboard_skips_unrevealed_active_quiz.sql`**: úplná definice `get_student_public_scoreboard` (produkční definice z 0012, md5 `809ca2e910f505b46b10bd467a012abe`) doplněná jen o načtení `revealed_block_ids` a jednu podmínku ve `scored_blocks`; signatura, `SECURITY DEFINER`, `search_path` a práva beze změny. Ověřeno na dočasné Neon větvi (kopie produkce, ukončená hodina přepnutá zpět do `live` s aktivním kvízem za 2 body a studentem se správnou odpovědí): před změnou 9/32 bez i se zveřejněnými výsledky kvízu, po změně 7/30 bez zveřejnění, 9/32 po zveřejnění, 9/32 po přechodu na další blok, 9/66 po skončení (stejně jako dřív), `null` při skrytém pořadí. Po odečtení nových řádků je text funkce shodný s produkcí (stejné md5). Funkce se volá přímým SQL (`readNeonStudentScoreboard`), ne přes Data API, schema cache netřeba obnovovat. **Migrace zatím není aplikovaná v produkci.** (2) `lib/scoreboard-server.ts` (učitelský přehled pořadí a Presenter) používá stejné pravidlo, načítá navíc `revealed_block_ids`. Kontroly: nový `scripts/verify-scoreboard-active-quiz.mjs` v `npm run check` (skutečné `loadTeacherScoreboard` proti falešnému klientovi: aktivní nezveřejněný kvíz se nepočítá, zveřejněný ano, po přechodu dál ano, po skončení vše; statická kontrola 0025 včetně LEGAL-021), `npx tsc --noEmit`, `npx next build`.

Aktualizováno: 2026-09-26 — **produkční nasazení a test 0.9.173** (bez změny verze). #419 (`640ebe4`) je v produkci: `www.syllonaut.com` obsluhuje nasazení `dpl_F9jT7NtzSMQmeTbZ4PbvwR9s75Tb` (Ready). **Test** (hodina `M5KA78V`, lekce „Karel Čapek…“, blok 5 s tabulkou o 6 řádcích a 4 sloupcích): student v mobilním viewportu 375 px ve vestavěném prohlížeči. Stránka má šířku 375 px (na 0.9.171 to bylo 580 px), místo tabulky je 6 karet s hodnotami 16 px, krátké údaje „Rok vydání“ a „Žánr“ jsou vedle sebe a delší „Výchozí problém“ přes celou šířku. Sloupec úkolu v řídicím centru učitele má na notebooku (okno 1470 px) šířku 555 px, takže i učitel vidí karty místo tabulky. **Rozhodnutí vlastníka:** to je v pořádku, hranice 560 px zůstává. Na hranici se ptal i kvůli velkým telefonům. Rozhoduje šířka bloku v CSS px, ne fyzické rozlišení, a telefony na výšku mají 360–440 CSS px.

Aktualizováno: 2026-09-26 — interní verze **0.9.173** (menší úprava, veřejná verze zůstává **0.9.170**): **datové tabulky v lekci na telefonu jako karty, bez zmenšené stránky.** Při produkčním testu 0.9.171 měla týmová mise se 4sloupcovou tabulkou na telefonu 375 px šířku layoutu 580 px. `components/LessonDataTable.tsx` dával tabulce `minWidth` `max(420, sloupce × 130)` px a obal s `overflow-x: auto` sice rolovat uměl, ale jeho obsah i tak roztáhl nadřazený blok a celou stránku. Telefon pak celou lekci zmenšil zhruba na 65 %, takže písmo tabulky vycházelo kolem 9 px. Nově je kořen tabulky `container-type: inline-size` (`components/LessonDataTable.module.css`), takže tabulka stránku už roztáhnout nemůže. Pod šířkou bloku 560 px (container query, rozhoduje šířka bloku, ne okna) se místo tabulky zobrazí **jedna karta na řádek**: hodnota z prvního sloupce tučně jako nadpis karty, ostatní hodnoty s názvem sloupce jako popiskem, v běžné velikosti písma (16 px). Krátké hodnoty (do 24 znaků, např. rok nebo žánr) jsou po dvou vedle sebe, delší text přes celou šířku. Prázdné buňky se vynechají. V širším bloku zůstává tabulka (u mnoha sloupců roluje uvnitř svého obalu). Titulek tabulky je jeden odstavec nad obojím a tabulku i karty pojmenovává přes `aria-labelledby`. Zobrazuje se vždy jen jedna varianta, takže čtečka obrazovky data nečte dvakrát. Tmavé řídicí centrum učitele má vlastní barvy karet. Platí pro studenta i náhled/editor lekce (`LiveBlock`, `LessonPreview`); PDF „Moje řešení“ se nemění. Ověřeno na dočasné stránce v `next dev`: 375 px bez rozšíření stránky, karty 16 px, na 1024 px tabulka. Kontroly: nový `scripts/verify-lesson-data-table.mjs` v `npm run check`.

Aktualizováno: 2026-09-26 — **produkční nasazení a test 0.9.172** (bez změny verze). #417 (`7d2836f`) je v produkci: `www.syllonaut.com` obsluhuje nasazení `dpl_3aGCg61eFBcs4NzuxeYp4BhjiZug` (Ready). **Test** (hodina `ZS3HPX9`, lekce „Karel Čapek…“, admin účet vlastníka, 2 týmy): učitel a Presenter v Chrome, student „Test C“ ve vestavěném prohlížeči v mobilním viewportu. Požadavky se počítaly přes `performance.getEntriesByType('resource')` od načtení Presenteru. Výsledky: (1) **Smyčka je pryč.** Presenter poslal za 69 s včetně startu, 3 přechodů bloků, odpovědi a ukončení hodiny 6 požadavků na `/api/sessions/[id]/presenter` (za prvních 18 s jen 1). Na 0.9.171 to bylo asi 3 za sekundu. Push upozornění se zpracovala přes Worker `/state` (7×). (2) Počet v kvízu šel 0 → 1 / 1 za 0,8 s po odpovědi, stále „Presenter“, nikdy „Záložní spojení“. (3) **Konec hodiny:** 0,4 s po „Ukončit hodinu“ projektor ukázal „Mise dokončena“ (krátce s „Připravuji projekci…“), za 1,24 s „Konečné pořadí“. Stav „Mise probíhá“ ani „Startovní zóna“ se už neobjevil (na 0.9.171 se „Mise probíhá“ vrátilo na ~1,2 s).

Aktualizováno: 2026-09-26 — interní verze **0.9.172** (menší úprava, veřejná verze zůstává **0.9.170**): **Presenter bez smyčky požadavků a bez problikávání fáze na konci hodiny.** Při produkčním testu 0.9.171 poslal Presenter asi 3 požadavky za sekundu na `/api/sessions/[id]/presenter` (561 za 190 s). V `components/PresenterMode.tsx` byla funkce `ui` při každém renderu nová, a protože je v závislostech `load`, efekt `[ensureLiveAccess, load]` se po každém `setData` spustil znovu a zavolal `load()`. Nově je `ui` přes `useCallback([english])`, takže platí zamýšlený rytmus: první načtení, 15s interval, push z Live Control, `visibilitychange` a 4s interval jen v režimu záložního spojení. Dále Presenter po prvním stavu `ended` ignoruje pozdější odpovědi s jiným stavem (hodina se už znovu neotevře), takže se na konci hodiny nevrací na „Mise probíhá“. `components/PresenterScoreboard.tsx`, který se zobrazuje jen u ukončené hodiny, při načítání ukazuje „Mise dokončena“ místo „Startovní zóna“. Kontroly v `scripts/verify-live-resilience.mjs`: žádná funkce vytvářená při každém renderu nesmí být v závislostech hooku v Presenteru (na kódu 0.9.171 kontrola selže kvůli `ui`), `ui` přes `useCallback` a ochrana stavu `ended`.

Aktualizováno: 2026-09-26 — **produkční nasazení a test 0.9.171 v živé hodině** (bez změny verze). #414 (`d69fe1f`) je v produkci: `www.syllonaut.com` obsluhuje nasazení `dpl_74wFVo8rZB9UzysiTUwdxWvtssDT` (Ready). **Test** (hodina `V88JHAR`, lekce „Karel Čapek: nalezený spis…“, admin účet vlastníka, 2 týmy): učitel a Presenter v Chrome, student A („Test A“, Tým 1) ve vestavěném prohlížeči v mobilním viewportu 375 × 812, student B („Test B“, Tým 2) v další záložce Chrome. Záznam změn DOM (MutationObserver) a požadavků (obal `window.fetch`) přímo ve stránkách. Výpadek API se simuloval odmítnutím `fetch` ve stránce, síť se neblokovala. Nic se neodevzdávalo k AI hodnocení. (a) **Student bez falešného výpadku:** A psal do týmového pole v bloku 5 a měl posunuto k poli. B mezitím 4× uložil koncept svého týmu. A dostal 5 push upozornění (5× Worker `/state`) a na každou sérii jeden sloučený primární `refresh()` asi za 3 s. Panel „Spojení se přerušilo“ se neobjevil ani na okamžik, odznak zůstal „Připojeno“. `scrollY`, pozice pole, rozepsaný text i fokus se nezměnily. (b) **Presenter:** po celou dobu „Presenter“, nikdy „Záložní spojení“. Počet v kvízu (blok 3) šel 0 → 1 → 2 / 2 do 0,5 s od odpovědi. (c) **Fallback:** po zablokování `/api/student/sessions/*` se u A objevil panel „Spojení se přerušilo…“ a odznak „Obnovuji spojení…“ po 7 s (pokusy každé 4 s). Po odblokování byl za 1,7 s zpět „Připojeno znovu“, za 4,2 s „Připojeno“ a panel zmizel. Po zablokování `/api/sessions/*/presenter` Presenter ukázal „Záložní spojení – hodina běží dál“ za 0,35 s, po odblokování byl za 0,8 s zpět na „Presenter“ (limit 15 s). (d) **Konec hodiny:** Presenter ukázal „Mise dokončena“ a „Konečné pořadí“ (1. Test B 1/16, 2. Test A 0/16) za 2,9 s po kliknutí na „Ukončit hodinu“. Cestou se na **asi 1,2 s znovu ukázalo „Mise probíhá“** (0,8–2,0 s po kliknutí). Dvakrát krátce problikla i „Startovní zóna – Připravuji projekci…“ (0,3–0,8 s a 2,0–2,9 s). **Nalezeno mimo scope, neopraveno:** (1) `PresenterMode` posílá asi 3 požadavky za sekundu na `/api/sessions/[id]/presenter` (561 za 190 s). `ui` je nová funkce při každém renderu, takže `load` (`useCallback` s `ui` v závislostech) se mění při každém renderu. Efekt `[ensureLiveAccess, load]` pak po každém `setData` znovu volá `load()`. Chování je starší než 0.9.171 a pravděpodobně způsobuje i problikávání fáze na konci hodiny. (2) Při týmové misi s datovou tabulkou (blok 5) měl mobilní viewport 375 px šířku layoutu 580 px (`innerWidth`/`scrollWidth`). Po skončení hodiny už 375 px; příčina neověřena.

Aktualizováno: 2026-09-26 — interní verze **0.9.171** (menší úprava, veřejná verze zůstává **0.9.170**): **živá hodina — upozornění z Live Control Workeru už nevyvolává falešný výpadek u studenta ani na projektoru.** Worker po každém uložení odpovědi pošle přes WebSocket upozornění všem připojeným. `components/StudentSession.tsx` na něj volal `refreshFromLiveControl()`, která nastavila `disconnectedRef` a stav `reconnecting`, takže se nad zadáním vložil panel „Spojení se přerušilo…“ a obsah poskočil i studentovi, který zrovna psal. `components/PresenterMode.tsx` volal `loadFallback()`, která přepnula projektor na „Záložní spojení – hodina běží dál“. Nově mají obě funkce parametr `source: 'fallback' | 'push'` (výchozí `'fallback'`). WebSocket volá variantu `'push'`: snapshot se použije potichu, stav spojení se nemění. `reconnecting` / `fallback` nastaví jen selhání primárního API (`/api/student/sessions/[id]`, `/api/sessions/[id]/presenter`); 4s interval ve fallback režimu Presenteru zůstává. Student po sérii upozornění naplánuje jeden sloučený `refresh()` z primárního API za 3 s (jeden časovač, zrušený v cleanupu efektu), aby se dál aktualizovala pole, která snapshot nenese (odhalené výsledky, pořadí, hodnocení). Kontroly: 4 nové v `scripts/verify-live-resilience.mjs` (push cesta a `reconnecting`/`fallback` jen při selhání primárního API, pro studenta i Presenter), `scripts/verify-live-student-projection.mjs` čeká volání `refreshFromLiveControl('push')`.

Aktualizováno: 2026-09-26 — **produkční nasazení 0.9.170 a Workeru 0.8.17, ověření v živé hodině** (bez změny verze). (1) **Vercel:** #412 (`22948d6`) je v produkci, GitHub deployment `6674884744` Production `success`, `www.syllonaut.com` obsluhuje nasazení Ready. (2) **Worker 0.8.17:** vlastník ho nasadil přes `npx wrangler deploy` z čisté kopie `origin/main` (`22948d6`) ve chvíli bez živé hodiny. Nové **Version ID `46616e06-83d9-4722-8938-ed0202ce1d9e`** (předchozí `95483978-ae95-4258-9e63-2a7698f99014`). `/health` vrací `workerVersion=0.8.17` a `protocolVersion=2`, `/state` bez tokenu vrací 401. Pozor: první pokus spustil vlastník v zastaralé kopii `~/Documents/…/neon-migration` (Worker 0.8.14). Ten skončil chybou `wrangler.jsonc` (`migrations` a `exports` naráz), takže se nic nenasadilo. **Worker nasazovat jen z čerstvé kopie `origin/main`.** (3) **Test v produkční hodině** (kód WPMJR8R, týmová lekce „Mise: Legrační dům“, 9 aktivit): učitel v Chrome. Student A („Test A“, Tým 1) běžel ve vestavěném prohlížeči. Student B („Test B“, Tým 2) jel přes API s vlastní cookie a capability, aby se cookie studentů nepřepsaly. Hlasování, kvíz a týmový úkol, ve kterém oba týmy uložily koncept; neodevzdávalo se, aby se nespustilo placené AI hodnocení. Každý student viděl v „Předchozí aktivity“ jen svou odpověď a text svého týmu: A „die Küche“ / „der Garten“ / TYM1, B „der Garten“ / „die Garten“ / TYM2. Číslování odpovídalo počítadlu „N / 9“. Worker `/state` s tokenem A i B vrátil jen vlastního účastníka, bloky do aktivního, `totalBlocks: 9`, `memberCount`, vlastní odpovědi a 0 events, bez stopy druhého studenta. WebSocket B přijal za celý test 10 zpráv, všechny jen `{type:'wake',revision}`. Po učitelově „Předchozí“ (6/9 → 5/9) se seznam zkrátil na 4 položky a otevřené položky zůstaly otevřené. Po přesunu na další blok zůstala sekce otevřená a nová položka přibyla sbalená. (4) **Nalezeno mimo scope, neopraveno:** `refreshFromLiveControl` v `StudentSession` nastaví na každou WebSocket zprávu stav „reconnecting“. Student proto po každé události učitele nebo spolužáka krátce vidí „Spojení se přerušilo…“, dokud ho neopraví další primární dotaz. Chování je starší než 0.9.170. Testovací lekce ukazuje i původní problém: týmový úkol odkazuje na „nabídku v předchozím bloku“ bez čísla a názvu (lekce vznikla před novým pravidlem).

Aktualizováno: 2026-09-26 — **produkční ověření 0.9.162 a 0.9.164** (#407 `d950684`, #410 `524e94e`, bez změny verze). `www.syllonaut.com` obsluhuje nasazení `dpl_GLbSTjrBYFi5mEZtdXo5wR8HnhzY` (Ready) z `524e94e`. Migrace `0022` a `0024` jsou v produkci, žádný uživatel nemá pauzu AI `refund`. Vlastník potvrdil, že účet `ceduk.info` (Free, stará plná refundace 199 Kč) po přihlášení už nemá banner „platba byla vrácena“ a AI funguje. `/cs/terms` i `/en/terms` ukazují VOP 1.13 účinné od 26. 9. 2026 s novým odstavcem článku 6 (plná refundace ukončí předplatné, kromě duplicitní platby). Zrušení předplatného po plné refundaci a uvolnění pauzy u duplicitní platby zatím neproběhly na skutečné refundaci (ověřeno testy a na dočasné Neon větvi); první ostrá refundace se ověří v logu Vercelu (`stripe full refund canceled subscription`, `stripe duplicate-payment refund kept subscription`). Předplatné `loubek@icloud.com` bylo zrušené už 26. 9. 03:53 UTC mimo aplikaci; zpětně se nic nerušilo.

Aktualizováno: 2026-09-26 — interní a veřejná verze **0.9.170**: **„Předchozí aktivity“ pro studenty v živé hodině**, **projekce stavu Live Control Workeru pro studenta** (Worker **0.8.17**, v repozitáři, **zatím nenasazený**) a **pravidlo navazování bloků v promptu**. Podrobnosti v sekci „Předchozí aktivity, projekce stavu pro studenta a navazování bloků 0.9.170“. Veřejná verze: ano (kritérium 1: student si během hodiny může otevřít už proběhlé aktivity a svou odpověď na ně).

Aktualizováno: 2026-09-26 — interní verze **0.9.164**: navazuje na 0.9.162 (body 8–10 schválené vlastníkem). (1) **Vrácení dvojité platby už AI nepozastaví na měsíc.** Dosud refundace s důvodem `duplicate` při běžícím předplatném AI pozastavila, a uvolnění (`try_release_individual_ai_billing_losses` z `sync_stripe_invoice_payment_event_v2`) počítá jen platby zaplacené *po* refundaci, takže čekalo až na další obnovení. Neon migrace **`0024_refund_pause_same_subscription_and_duplicate_release.sql`**: (a) větev `'refund'` v `private.individual_ai_billing_pause_reason` vyžaduje, aby bylo živé právě *refundované* předplatné (join přes `external_subscription_id`); vrácení platby druhého, duplicitního předplatného tak nepozastaví AI na prvním; (b) nová serverová funkce `public.release_stripe_duplicate_payment_refund(event, livemode, charge, event_at)` (`SECURITY DEFINER`, `search_path=''`, žádná práva pro API role, allowlist v `lib/neon/billing-rpc.ts`) uvolní záznam refundace s `release_reason = 'duplicate_payment'` (rozšířené omezení `individual_billing_refunds_release_reason_check`), jen když má totéž předplatné jinou platbu v téže měně, aspoň ve výši vrácené částky, sama plně nevrácenou, zaplacenou do 7 dnů od vrácené platby; jinak vrátí `no_covering_payment` a platí stávající pravidla. Webhook ji volá, když jsou všechny účinné refundace dané charge `duplicate`; smíšené důvody (`mixed_refund_reasons`) nic neruší ani neuvolňují a jen se zalogují. `sync_stripe_refund_state` při každé další refundační události plnou refundaci znovu otevře, webhook proto uvolnění při každé duplicitní události zopakuje; chyba → 500. Ověřeno na dočasné Neon větvi (smazané): výchozí stav beze změny, jiné živé předplatné + zrušené refundované → `null`, živé refundované → `refund`, bez pokrývající platby i s pěti nevyhovujícími (nízká částka, po 7 dnech, jiná měna, jiné předplatné, sama vrácená) → `no_covering_payment`, s pokrývající → uvolněno a pauza `null`, opakování → `already_released`, opětovné otevření přes `sync_stripe_refund_state` a nové uvolnění, práva jen vlastník DB. **Aplikovaná v produkci 2026-09-26** po souhlasu vlastníka (jedna transakce, 7 příkazů; md5 obou funkcí shodné s ověřenou dočasnou větví, práva jen vlastník DB, žádný uživatel nemá pauzu `refund`). Schema cache Data API netřeba obnovovat (funkce se volají přímým SQL, ne přes Data API). (2) **Nápověda U3** (CS/EN, `lib/help/knowledge.ts`): plné vrácení platby za aktuální období předplatné ukončí a účet přejde na Free; vrácení dvojité platby předplatné neukončí. (3) **VOP 1.13** (`2026-09-26-v14`, účinné od 26. 9. 2026) uzavírá **LEGAL-024**: článek 6 doplněn o `TERMS_FULL_REFUND_CLAUSE` (`lib/terms-content.ts`, stejný text na stránce VOP i v neměnném snímku smlouvy, snímek připnutý na 1.13). Rozhodnutí vlastníka 26. 9.: změna se týká jen plného vrácení peněz na žádost uživatele, neprošla posouzením právníkem, souhlasy v4–v13 zůstávají dostatečné pro běžné užívání a nové registrace a objednávky zaznamenají 1.13; oznámení e-mailem podle čl. 10 se neposílá. Kontroly: `verify-full-refund-subscription-cancellation` (duplicitní uvolnění, jeho opakování, chyba → 500, smíšené důvody, statická kontrola 0024), `verify-terms`, `verify-provider-contact`, `verify-ai-grading-topups`. Veřejná verze: ne (platby a fakturace, právní dokument).

Aktualizováno: 2026-09-26 — **produkční ověření zápisu 0.9.163** (bez změny verze). Vlastník zavřel upozornění na Free účtu tlačítkem „OK, rozumím“. V `private.free_single_use_notice_acknowledgements` vznikl první řádek (26. 9. 2026 04:50:30 UTC): `dismissed_via = ok`, `locale = cs`, `notice_version = 2026-09-26`, účet existuje s tarifem `free`, `lesson_id` uložené a lekce patří tomuto učiteli, `notice_text` se shoduje s konstantou `FREE_SINGLE_USE_NOTICE.cs` na `main` (211 znaků, stejné md5). Po obnovení stránky se bublina na tomto účtu už neukáže (potvrdil vlastník). Neověřeno zvlášť: zavření křížkem (`close`), které volá stejnou funkci.

Aktualizováno: 2026-09-26 — **produkční ověření 0.9.163** (#408, `681a38b`, bez změny verze). Produkční nasazení z `681a38b` je Ready. `/cs/gdpr` a `/en/gdpr` ukazují Privacy Notice 1.10 (účinná od 26. 9. 2026) s položkou o potvrzení upozornění Free. `POST /api/lessons/[id]/free-single-use-notice` bez přihlášení vrací 401. Produkční katalog po migraci 0023 (čtení povolené vlastníkem): RLS zapnuté, trigger `free_single_use_notice_ack_append_only`, žádná práva `anon`/`anonymous`/`authenticated`/`authenticator`/`PUBLIC` na tabulku ani sekvenci, funkce triggeru se `search_path=""` a bez EXECUTE pro `authenticated`, 0 řádků. Neověřeno v UI: zavření bubliny skutečným Free účtem (vlastník Free účet nemá); první skutečný záznam v tabulce potvrdí zápis.

Aktualizováno: 2026-09-26 — interní verze **0.9.163**: **upozornění Free „každou lekci živě použít jednou“ jde zavřít a zavření se eviduje** (požadavek vlastníka). Plovoucí bublina u tlačítka „Otevřít hodinu pro studenty“ (`components/StartSessionButton.tsx`) zabírala rozhraní a na menších šířkách překážela proklikávání náhledu. Nově má křížek vpravo nahoře a tlačítko „OK, rozumím“ („OK, got it“). Obojí bublinu hned zavře a pošle `POST /api/lessons/[id]/free-single-use-notice` (`dismissedVia` `ok`/`close`, `locale`). Server (`lib/free-single-use-notice-ack.ts`) zapíše append-only řádek do **`private.free_single_use_notice_acknowledgements`**: ID učitele, lekce (jen pokud je jeho), verze upozornění, jazyk, **přesný text upozornění ze serverové konstanty** (ne z požadavku), způsob zavření a čas serveru. Záznam slouží jako důkaz pro případnou reklamaci „nevěděl jsem, že lekci spustím jen jednou“. Detail lekce upozornění už nezobrazí, pokud učitel potvrdil jeho aktuální verzi (`FREE_SINGLE_USE_NOTICE_VERSION` v `lib/free-single-use-notice.ts`, dnes `2026-09-26`). Při změně textu se musí změnit verze a upozornění se ukáže znovu. Když zápis selže, bublina se zavře jen do dalšího načtení stránky. Upozornění v knihovně „Moje lekce“ se nemění (není plovoucí). **Neon migrace `0023_free_single_use_notice_acknowledgements.sql`**: tabulka jen pro server (RLS, bez práv pro API role), bez FK na účet a lekci (jako `private.terms_acceptance_events`, aby důkaz nezmizel se smazáním), trigger odmítá UPDATE i DELETE. Vyzkoušeno na dočasné větvi z produkce: zápis s vlastní lekcí uloží `lesson_id`, cizí lekce se uloží jako `null`, čtení potvrzení podle verze funguje, UPDATE/DELETE odmítnuté, žádná práva pro `anon`/`anonymous`/`authenticated`/`authenticator`. Schema cache Data API se obnovovat nemusí (tabulka se volá jen přes serverové SQL). Před aplikací migrace se čtení tiše vrátí k zobrazení upozornění a zápis vrátí 500. **Aplikováno v produkci 2026-09-26** před sloučením PR (Neon konektor, jedna transakce, 12 příkazů; starší build tabulku nepoužívá), dočasná větev smazaná. Kontrolní čtení produkčního katalogu (RLS, trigger, práva) zablokoval auto-mode klasifikátor; ověřeno jen na dočasné větvi. `verify-free-lesson-reuse` hlídá obě tlačítka, serverový zápis, text ze serverové konstanty a append-only tabulku. **Privacy Notice 1.10** (`/gdpr`, účinná od 26. 9. 2026): v sekci 2 položka „Potvrzení upozornění na jedno živé použití lekce ve Free“, v sekci 3 účel a právní základ (oprávněný zájem na doložení poskytnutých informací a obhajobě právních nároků), v sekci 9 uchování i po zrušení účtu po dobu přiměřeně nutnou k vyřízení reklamací a obhajobě právních nároků. DPA beze změny (jde o údaje učitele, ne studentů). `verify-privacy` hlídá verzi 1.10 a nové texty. Veřejná verze: ne (menší úprava).

Aktualizováno: 2026-09-26 — interní verze **0.9.162**: **plná refundace individuálního předplatného ho okamžitě ukončí** a AI pauza po refundaci platí jen při živém předplatném (rozhodnutí vlastníka). Dosud webhook plnou refundaci jen zapsal do `private.individual_billing_refunds`, takže předplatné ve Stripe běželo dál, a uživatel na Free měl AI trvale pozastavené s bannerem „platba byla vrácena“ (VOP čl. 6 dovolují jen dočasně omezit nové *placené* AI operace). (1) **Webhook** (`app/api/billing/stripe/webhook/route.ts`, logika `lib/stripe-full-refund-cancellation.ts`): po úspěšném `sync_stripe_refund_state` (jen individuální platby, jen LIVE, jen plná refundace; školy a balíčky AI hodnocení beze změny) načte ze Stripe refundace charge, předplatné a fakturu refundované platby (`invoice_payments`). Předplatné `trialing`/`active`/`past_due` zruší stejně jako online odstoupení (`cancelWithdrawnSubscription`: `DELETE`, `invoice_now=false`, `prorate=false`; nově volitelný idempotency key, pro odstoupení beze změny), jen když refundovaná platba patří k jeho `latest_invoice`. Neruší: důvod `duplicate`, částečná / neúspěšná / zrušená refundace, refundace starší faktury (varování v logu), refundace z odstoupení nebo změny služby (metadata `syllonaut_withdrawal_id` / `syllonaut_service_change_termination_id`). Opakované `charge.refunded`, `refund.created`, `refund.updated` najdou předplatné zrušené. Selhání Stripe → 500 a Stripe událost zopakuje. Nový e-mail ani Resend událost nejsou: „Předplatné bylo ukončeno“ a `syllonaut.subscription.ended` pošle existující cesta `customer.subscription.deleted`. Oprávnění klíče „Syllonaut Live“ stačí stávající (Charges and Refunds, Invoices Read, Subscriptions Write). (2) **Neon migrace `0022_refund_pause_requires_live_individual_subscription.sql`** (`0020`/`0021` už obsazené): `private.individual_ai_billing_pause_reason` vrací `'refund'` jen se zároveň živým individuálním předplatným; ostatní větve, pořadí, podpis, `SECURITY DEFINER`, `search_path` a práva beze změny, data refundací beze změny. Zkouška v produkci 26. 9. v jednom příkazu s vynuceným rollbackem: `ceduk.info` `refund` → `null`, `loubek@icloud.com` `refund` → `null` (jeho předplatné už bylo zrušené 26. 9. 03:53 UTC mimo aplikaci) a se simulovaně aktivním předplatným `refund`, admin `null` → `null`; jiné účty změna nezasáhne. **Aplikovaná v produkci 2026-09-26** po souhlasu vlastníka (Neon konektor, jedna transakce, 2 příkazy); ověřeno: nové tělo funkce, `SECURITY DEFINER`, `search_path=''`, práva jen vlastník DB, všechny tři účty vrací `null`. Schema cache Data API netřeba obnovovat (privátní funkce, stejný podpis). (3) Provozní pravidlo pro refundace ve Stripe dashboardu (vždy vybrat důvod: dvojitá platba **Duplicate**, jinak **Requested by customer**) je v `docs/NEON_MIGRATION.md`. Starší refundace se zpětně nezpracují. Kontroly: nový `scripts/verify-full-refund-subscription-cancellation.mjs` v `npm run check` spouští skutečnou webhook route s podepsanými událostmi a falešným Stripe (plná refundace aktuální faktury, `duplicate`, částečná, starší faktura, opakované události, odstoupení a změna služby, neúspěšná a zrušená refundace, selhání Stripe → 500 a zopakování, školy a balíčky beze změny) a staticky migraci 0022. Veřejná verze: ne (platby a fakturace).

Aktualizováno: 2026-09-25 — Resend (Cowork, bez změny verze): šablony `lifecycle-grading-quota-teacher-cs` (`1c37c93b-8eaf-4560-985c-89412b60b73c`) a `-teacher-en` (`1f927a0b-59b0-421f-aac2-89d869000da7`) mají blok „Potřebujete více návrhů hodnocení od AI?“ / „Need more AI grading suggestions?“: jen s aktivním Teacher Pro, 60/100/200 návrhů za 99 Kč / 3,99 € / $4.49, 149 Kč / 5,99 € / $6.49, 279 Kč / 11,49 € / $12.49 (formát jako Ceník), platnost 12 měsíců, čerpání až po limitu tarifu. Hlavní CTA `https://www.syllonaut.com/{cs|en}/subscription?utm_…&utm_content=grading_quota_teacher_{cs|en}_v2#dokoupit`, vedlejší odkaz /lessons. Úvod doplněn o „…a nemáte k dispozici žádné dokoupené návrhy“ (oznámení vzniká až po vyčerpání tarifu i balíčků, migrace 0019). Školní šablony beze změny. Test na review kontakt: CS učitel doručen, blok i proměnné správně; EN zkontrolována v šabloně. Známé omezení: oznámení je jedno na účet a období (unique v `private.ai_grading_quota_notices`), po dokoupení a opětovném vyčerpání ve stejném období druhý e-mail nepřijde.

Aktualizováno: 2026-09-25 — **UI ověření 0.9.161 v lokálním harnessu** (bez změny verze). Skutečná komponenta `SubscriptionManagement` z `main` (`16dc39f`) se stavem `{ kind: 'none' }` na dočasné stránce v `next dev` (jen ve scratch kopii, necommitováno); odpověď `/api/auth/devices` podvržená v prohlížeči. (1) Politika platí (`required: true`, `personal`, 3/3, `trusted_device_limit_reached`): pod kartou „Používáš tarif Free“ se zobrazí panel „Důvěryhodná zařízení“ s 3/3, hláškou o limitu a tlačítky „Odebrat“. (2) „Odebrat“ pošle `DELETE /api/auth/devices` se správným `deviceId`; panel se překreslí podle odpovědi (zařízení zmizí, přibude „Toto zařízení“, hláška zmizí). (3) Politika neplatí (`required: false`, skutečný Free účet): panel se nezobrazí, zůstane jen karta Free a odstoupení. **Neověřeno:** skutečný server a databáze (odebrání přes endpoint a následné spuštění hodiny; beze změny od 0.9.161) — potvrdí až akce účtu s výjimkou Teacher Pro v produkci.

Aktualizováno: 2026-09-25 — **test banneru balíčků v živé hodině na Vercel Preview** (bez změny verze). Na pokyn vlastníka přidán `AI_GRADING_TOPUPS_ENABLED=true` do Vercel **Preview** (Production beze změny) a znovu sestaveno Preview z kódu `main` (`edupilot2-7s9v2l8cd`), které běží proti Neon větvi `preview`. Vlastník si tam zaregistroval testovací účet `vaclav.loubek+banner@gmail.com`; na `preview` jsem mu vložil ukázkovou lekci z `lib/demo.ts` s rubrikami u bloků 1–3. Hodinu jsem otevřel v UI a student se připojil v druhé záložce. (1) Odpověď na blok 1 s tarifem Free → `manual-v1` (bez volání AI). (2) Potom účet dostal Teacher Pro přes `manual_entitlement_overrides` a 60 použitých návrhů tarifu; odpověď na blok 2 → `manual-budget-v1`, zapsané `ai_grading_quota_notices`. V řídicím centru se ukázal banner „Návrhy hodnocení od AI jsou pro toto období vyčerpané (60 z 60)… Limit se obnoví 1. 10. 2026.“ s odkazem „Dokoupit návrhy“ (`/cs/subscription#dokoupit`, nová záložka); u odpovědi text „Čeká na ruční hodnocení – limit návrhů od AI je vyčerpaný.“ (3) Po vložení testovacího LIVE balíčku (60) banner ukázal „Máš k dispozici 60 dokoupených návrhů hodnocení od AI. 1 odpověď čeká na ruční hodnocení kvůli vyčerpanému limitu.“ a tlačítko „Požádat AI o návrhy pro 1 čekající odpověď“. Po kliknutí banner zmizel, odpověď přešla do `grading` a skutečné AI hodnocení (sdílený klíč Preview, 0,015 USD) dalo návrh 4/6; rezervace se čerpala z balíčku (`succeeded`), ne z tarifu, a v řídicím centru se objevilo „1 návrh AI čeká na tvoje potvrzení“. Testovací data jsem potom z `preview` smazal (hodina, lekce, balíček, rezervace, oznámení, ruční Teacher Pro); účet zůstal jako Free, `AI_GRADING_TOPUPS_ENABLED` v Preview zůstává zapnutý (odpovídá produkci). Poznámka k `preview`: při registraci přijdou dva kódy od „Neon Auth“ a platí jen novější. Neon zřejmě pošle kód už při registraci (webhook e-mailů je na `preview` vypnutý) a aplikace pak pošle vlastní. Produkce posílá jen jeden e-mail (ověřeno v Resendu).

Aktualizováno: 2026-09-25 — **produkční nasazení 0.9.161** (#402, `35b821e`, bez změny verze): `www.syllonaut.com` a `syllonaut.com` obsluhuje nasazení `edupilot2-gjmnofosb-vaclavloubek1.vercel.app` (`dpl_DbKQfqx6EFfMrezWUEbkACeJrFUi`, stav Ready). GitHub deployment `6659103972` (Production, `success`) patří ke commitu `35b821e`; ověřeno přes `vercel-cli inspect www.syllonaut.com` a `gh api …/deployments?sha=…`. **Za provozu zatím neověřeno:** že účet s výjimkou Teacher Pro (3/3 zařízení) vidí na `/cs/subscription` panel důvěryhodných zařízení, odebere zařízení a spustí hodinu; vyžaduje přihlášení toho účtu.

Aktualizováno: 2026-09-25 — interní verze **0.9.161**: účet s placeným tarifem z ruční výjimky (`manual_entitlement_overrides.plan_code`, bez předplatného ve Stripe) si na `/subscription` může spravovat důvěryhodná zařízení. Serverová politika zařízení (`personal_trusted_device_policy_required`, `active_plan_code` `teacher`/`teacher_pro`) pro něj platí, ale `components/SubscriptionManagement.tsx` pro `state.kind === 'none'` vykresloval jen kartu „Používáš tarif Free“ bez `<TrustedDevicesPanel />`, přestože hláška při zablokování posílá uživatele do správy předplatného. Produkční případ: účet s výjimkou Teacher Pro měl 3/3 zařízení, nemohl odebrat žádné a nemohl spustit hodinu. Větev bez aktivního předplatného teď vykresluje `<TrustedDevicesPanel />` mezi kartou a panelem odstoupení; panel se sám skryje, když `summary.required === false`, takže skuteční Free uživatelé nic nového nevidí. Větev admin beze změny (admin politiku zařízení nemá). Text karty „Používáš tarif Free“ se zatím nemění (zdokumentované chování 0.9.117, řeší se zvlášť). Regresní kontrola v `scripts/verify-trusted-devices.mjs` ověřuje, že větev `if (!active)` vykresluje `<TrustedDevicesPanel />` (bez opravy selže). Kontroly `npx tsc --noEmit`, `npm run check`, `npm run check:accessibility`, `npx next build`. Veřejně zobrazovaná verze: ne.

Aktualizováno: 2026-09-25 — **testovací LIVE nákup balíčku návrhů hodnocení od AI** (bez změny verze). Vlastník na ne-admin účtu `loubek@icloud.com` (role `user`) zaplatil LIVE **Teacher Pro měsíčně 329 Kč** (CZ/CZK, Apple Pay nebo karta) a potom balíček **60 návrhů za 99 Kč**; souhlasy zaškrtl a platby zadal sám. (1) **Grant:** `private.ai_grading_credit_grants` `73bbce97-…` — `livemode = true`, `grading_60`, 60 návrhů, `amount_minor` 9900 CZK, `purchased_at` 2026-09-25 09:34:55 UTC, `expires_at` 2027-09-25, se snímkem smlouvy a `external_subscription_id` předplatného. (2) **UI s ne-admin účtem:** `/cs/subscription#dokoupit` ukazuje sekci „Dokoupit návrhy hodnocení od AI“ (3 souhlasy, 99 / 149 / 279 Kč, 1,65 / 1,49 / 1,40 Kč za návrh, „Cena je konečná, nejsme plátci DPH.“), souhrn po nákupu „60 z 60 zbývá · + 60 dokoupených“; panel „Využití AI“ na `/cs/lessons` „Návrhy hodnocení od AI · zbývá 60 z 60 · + 60 dokoupených (platné do 25. 9. 2027)“, odkaz „Dokoupit“ správně skrytý (zbývá > 10). Tím je panel „Využití AI“ poprvé ověřený s ne-admin účtem; banner v hodině a tlačítko „Požádat AI o návrhy …“ zůstávají za provozu neověřené (vyžadují živou hodinu po vyčerpání tarifu). (3) **Čerpání** na dočasné Neon větvi z produkce se skutečným grantem (`tmp/topup-golive-consumption`, `br-winter-leaf-b2iffp6i`): po 60 použitých návrzích tarifu vrátila `private.reserve_ai_grading_budget` pro další 3 hodnocení `true` a všechny 3 rezervace čerpaly z balíčku `73bbce97-…`; `ai_grading_quota_notices` se nezapsalo. (4) **Refund:** vlastník vrátil ve Stripe obě platby (Stripe před refundem vyžaduje ověření přes Google; v záložce ovládané rozšířením Claude selhalo, v běžném okně prošlo). Webhook zpracoval všechny 4 události s 200: balíček `revoked_at` 2026-09-25 10:03:01 UTC, `revoke_reason = 'refund'`, „+ 60 dokoupených“ i sekce Dokoupit zmizely; refund předplatného podle návrhu pozastavil AI („Platba vrácena“), předplatné ve Stripe zůstává `active` s obnovou 25. 10. 2026 (zrušení je na vlastníkovi). (5) **Nalezeno:** `invoice.paid` nového předplatného (`evt_1UJVSkArYiDuwS3OGZWgMJuv`) přišla ve stejné sekundě jako `customer.subscription.created` a `sync_stripe_invoice_payment_event_v2` vrátila `stripe_invoice_subscription_mapping_mismatch` (P0001) → 500; Stripe událost zopakoval a platba 329 Kč je v `private.stripe_subscription_payments` od 09:30:56 UTC (18 s po chybě). Beze ztráty dat, ale každé nové předplatné může v logu i ve Stripe zanechat jedno neúspěšné doručení; neopravováno (mimo rozsah). Fakturu balíčku (99 Kč) zkontroloval vlastník: částka, popis balíčku i číselná řada sedí. Banner v hodině ověřen 25. 9. na Vercel Preview (viz záznam o testu banneru). (6) **Úklid:** vlastník testovací předplatné Teacher Pro zrušil; dočasná Neon větev `br-winter-leaf-b2iffp6i` smazána. (7) **Patička „Dodavatel není plátcem DPH“ na fakturách Stripe: rozhodnutí vlastníka ji nedávat.** Výchozí patička účtu (Settings → Billing → Invoices → Default footer) by platila pro všechny doklady a Stripe nedokumentuje, zda se nepropíše i na doklady Managed Payments (EUR/USD, prodejcem je Link a účtuje DPH); vlastník nechce, aby ji viděli EUR a USD zákazníci. Krátce uložená LIVE patička byla smazána dřív, než se vystavila jakákoli faktura; LIVE i sandbox mají Default footer prázdný. Informace „nejsme plátci DPH“ zůstává jen v aplikaci (sekce Dokoupit).

Aktualizováno: 2026-09-25 — **audit A1: rozhodnutí vlastníka a krok 1 (Preview a lokální vývoj na Neonu)**, bez změny verze. **Rozhodnutí:** supabasové cesty v kódu (69 souborů) se budou odstraňovat postupně po oblastech (lekce → živá hodina → billing a organizace → přihlášení a komponenty → přejmenování `lib/supabase/*` a odebrání `@supabase/*`), a to **nejdřív od 2026-10-23**, tedy po měsíci stabilního provozu na Neonu. Do té doby zůstává Supabase jen pro čtení jako záloha. **Krok 1 hotový teď:** nová Neon větev **`preview`** (`br-green-bar-b2q894po`). Neon v projektu nedovolí větev „jen schéma“ (starší role `service_role`), proto vznikla jako kopie produkce a byla ihned vyčištěna: smazána všechna uživatelská data ve schématech `public`, `private`, `app_identity` a `neon_auth` i celé schéma `migration_backup_20260923083128`. Zůstal jen katalog `billing_plans` (7) a testovací `billing_prices` (13, `livemode = false`); počty řádků ověřeny. Starší stav větve je dostupný jen vlastníkovi projektu přes obnovu k bodu v čase a po 6 h zmizí. Neon Auth větve `preview` má vlastní endpoint (`ep-cool-fire-…`) a **nový podpisový klíč** (zkopírovaný produkční `jwks` smazán; `kid` se od produkce liší), webhook e-mailů na produkci je na `preview` vypnutý (ověřovací kódy posílá Neon), z povolených originů odebrány `www.syllonaut.com` a `syllonaut.com` a přidáno `https://*.vercel.app` (rozhodnutí vlastníka). Každé Preview nasazení má novou adresu a Neon Auth jiný zástupný vzor nepřijme; bez toho registrace končila `INVALID_ORIGIN`. Riziko je omezené, protože větev obsahuje jen testovací účty a Preview je za přihlášením do Vercelu. Data API větve má stejné nastavení jako produkce a po výměně klíče je obnovené. **Vercel Preview (všechny větve):** `NEON_DATABASE_URL`, `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `NEON_AUTH_BASE_URL`, `NEXT_PUBLIC_NEON_AUTH_URL`, `VITE_NEON_AUTH_URL`, `NEON_DATA_API_URL` a `NEXT_PUBLIC_NEON_DATA_API_URL` míří na větev `preview`. Proměnné z integrace Vercel↔Neon dříve nejspíš mířily na produkční větev; CLI je kvůli stejnojmenným proměnným větve `codex/neon-staging-import-20260921-v2` neumělo přepsat, opraveno přes Vercel API podle ID. Dále nový `NEON_AUTH_COOKIE_SECRET`, `DATABASE_BACKEND=neon`, `NEXT_PUBLIC_DATABASE_BACKEND=neon`, `NEON_CUTOVER_APPROVED=true` a testovací klíče Turnstile. Production se nemění. **V repozitáři:** `lib/neon/turnstile.ts` přijímá testovací tajný klíč Cloudflare (odpověď má hostname `example.com` bez action) jen mimo produkci (`VERCEL_ENV !== 'production'`); `.env.example` a README popisují lokální vývoj proti větvi `preview`; `verify-neon-migration-preparation` hlídá obojí. **Ověřeno 2026-09-25 na Preview PR #399:** klientský JavaScript volá jen endpoint větve `preview` (`ep-cool-fire`), ne produkční `ep-green-hat`. Vlastník se zaregistroval (Turnstile s testovacím klíčem, ověřovací kód od Neon Auth). Na `preview` vznikl ověřený uživatel, identita, profil Free, souhlas s VOP i marketingový souhlas; v produkci nepřibyl žádný uživatel. Běžné PR Preview tím testují stejné Neon cesty jako produkce a nemají produkční přihlašovací údaje k DB. SEC-002 zůstává otevřený pro sdílený `AI_GATEWAY_API_KEY`.
...
Aktualizováno: 2026-09-25 — interní verze **0.9.160**: dialog výběru fakturační země v Ceníku se už neposouvá celý, jak to od 0.9.159 dělal (vlastníkovi se to nelíbilo). Poučení o odstoupení (`TERMS_WITHDRAWAL_CLAUSE`) je nově v rámečku s vlastním posouváním (`.checkoutLegal`, `role="region"` s popiskem, dosažitelné klávesnicí přes `tabIndex={0}`, zvýrazněný fokus). Dialog je sloupcový flex s `max-height: calc(100dvh - 40px)` (≤ 540 px: `- 20px`): ostatní části se nezmenšují, zmenšuje se jen rámeček s poučením (nejméně 96 px); když se poučení vejde, zobrazí se celé bez posuvníku. Celý dialog se posune jen jako pojistka, když se nevejde ani s nejmenším rámečkem (např. telefon na šířku 844 × 390). Text ani pořadí souhlasů se nemění. Ověřeno kopií dialogu se zkompilovaným CSS v prohlížeči: 1280 × 1100 (poučení celé), 1024 × 768 a 402 × 874 (dialog bez posouvání, tlačítka vidět, poučení se posouvá), 844 × 390 (pojistka). 0.9.159 (#396) byla v produkci ověřená čtením CSS z `/cs/pricing` (nasazení z `9e488ab`). Kontroly `npx tsc --noEmit`, `npm run check`, `npm run check:accessibility`, `npx next build`. Veřejně zobrazovaná verze: ne.
...
Aktualizováno: 2026-09-25 — **spuštění balíčků návrhů hodnocení od AI** (provozní kroky, bez změny verze). (1) Stripe (na pokyn vlastníka, v jeho Chrome): LIVE webhook `we_1UHHNRArYiDuwS3OzTA8LueC` nově odebírá i `checkout.session.completed` a `checkout.session.async_payment_succeeded` (18 událostí), sandbox destinace `we_1UH4wTAgkxhGI3t1wBRaZSAs` (`acct_…AgkxhGI3t1`) tytéž dvě (9 událostí; refundy a spory sandbox destinace dál neodebírá). Checkouty předplatného webhook dál ignoruje (`normalizeStripeTopupCheckoutEvent` vrací `null` bez metadat balíčku). (2) LIVE klíč „Syllonaut Live“ stačí: nákup balíčku volá jen `POST /v1/checkout/sessions` (Checkout Sessions Write), refund Charges and Refunds; ceny se čtou z `billing_topup_prices`. (3) `AI_GRADING_TOPUPS_ENABLED=true` přidán do Vercel Production (rozhodnutí vlastníka: zapnout globálně bez allowlistu, protože v produkci nemá nikdo LIVE Teacher Pro a admin je z nákupu vyloučen, takže nákup reálně uvidí jen testovací ne-admin účet). Převzalo ho nasazení `dpl_Gwcg4UJgWuHAM58sfTrCyfjqhStU` (z `e76ad1d`), redeploy nebyl potřeba. Ověřeno bez přihlášení: `POST /api/billing/stripe/topup/checkout` vrací 400 místo 404, `/cs/pricing` a `/en/pricing` ukazují u Teacher Pro blok „Potřebujete více návrhů hodnocení od AI?“ / „Need more AI grading suggestions?“ se všemi 9 cenami, 10 minut po nasazení žádná odpověď 500. **Zbývá:** testovací LIVE nákup s ne-admin účtem (Teacher Pro, pak balíček 60 za 99 Kč; platby zadává vlastník) → grant v DB, `/subscription#dokoupit`, panel „Využití AI“ a banner s ne-admin účtem, faktura → čerpání tarif → balíček ověřit na dočasné Neon větvi z produkce → refund balíčku → zrušení zbytku; blok v Resend šabloně „Grading quota reached“ (Cowork).

Aktualizováno: 2026-09-25 — **migrace 0021 aplikovaná v produkci** (bez změny verze, audit B6): `private.marketing_lifecycle_failures` vytvořená v jedné transakci po souhlasu vlastníka. Ověřeno: tabulka existuje, RLS zapnuté, žádná práva pro `anon`/`anonymous`/`authenticated`/`authenticator`, kontroly `operation_check` a `code_check` na místě, 0 řádků. Tabulka se přes Data API nevolá, obnova schema cache proto nebyla potřeba. Produkce obsluhuje `dpl_HTiWqWXpKo6BELqdzssAkvQVpQm5` (Ready) ze sloučení #394 (`e0c93ea`), takže zápis chyb Resendu a denní souhrn (`/api/cron/marketing-failures`, 06:57 UTC) jsou aktivní. Z auditu zbývají A1 (supabasové větve) a C1–C9 (dokumentace).

Aktualizováno: 2026-09-25 — interní verze **0.9.158**: denní souhrn chyb marketingového lifecycle e-mailem (audit B6, rozhodnutí vlastníka). Chyby volání Resendu z `lib/marketing-lifecycle.ts` končily jen v logu Vercelu a část z nich (volání přes `Promise.allSettled`) ani tam. Výpadek `401 restricted_api_key` z 22.–23. 9. proto nikdo nezaznamenal. Nově `resendRequest` a obě kontroly odpovědi kontaktu každou chybu nejdřív zapíšou do `private.marketing_lifecycle_failures` (`lib/marketing-failures.ts`). Záznam je po dnech (UTC), operaci (metoda + vzor cesty Resendu, například `POST /events/send` nebo `GET /contacts/:email`, nikdy e-mail) a kódu chyby, s počtem a prvním a posledním výskytem. Zápis nikdy nevyhodí chybu. Nový cron `/api/cron/marketing-failures` (denně 06:57 UTC, chráněný `isAuthorizedCronRequest`) pošle na `PROVIDER_CONTACT.email` souhrn přírůstků od posledního hlášení (`failure_count > reported_count`) s vysvětlením kódů a smaže záznamy starší 90 dní. Bez chyb e-mail nechodí. Souhrn jde přes `sendResendEmail` stejným `RESEND_API_KEY`: omezený klíč jen pro odesílání souhrn doručí, úplně chybějící klíč ne (cron pak skončí 500 v logu Vercelu). **Neon migrace `0021_marketing_lifecycle_failures.sql`** (tabulka jen pro server, RLS, bez práv pro API role, bez e-mailu a ID uživatele) je vyzkoušená na dočasné větvi, poté smazané: sčítání, hlášení jen přírůstků, odmítnutí operace s e-mailem, `permission denied` pro `authenticated`. **V produkci čeká na aplikaci.** Do té doby se zápis tiše přeskočí (jen `console.error`) a cron vrátí `available: false`. `verify-marketing-lifecycle` hlídá zápis u každé chyby Resendu, anonymizaci cesty, hlášení přírůstků, ochranu a plán cronu a práva tabulky. Chyby vlastních DB dotazů (`marketing_*_lookup_failed`) souhrn nezahrnuje. **Audit B5 dokončen:** migrace `0020` (odstranění `reconcile_live_control_events`) aplikovaná v produkci 2026-09-25 po sloučení #393, funkce zmizela, `reconcile_live_control_snapshot` a `_impl` zůstaly, schema cache Data API obnovená se stávajícím nastavením. **Audit B1:** ponecháno (`package.json` 0.3.0 se nepoužívá; `lib/version.ts` je záměrně veřejná verze). Veřejná verze: ne (provozní upozornění).

Aktualizováno: 2026-09-25 — **připravená Neon migrace 0020** (audit B5, bez změny verze, **v produkci zatím neaplikovaná**): `neon/migrations/0020_drop_live_control_events_reconciliation.sql` odstraní `public.reconcile_live_control_events(uuid, jsonb)`. Funkce je pozůstatek event-based reconciliace ze Supabase (`supabase/migrations/20260918050036`). Nevolá ji aplikace, Live Control Worker ani jiná DB funkce; živá hodina se rekonciliuje přes `reconcile_live_control_snapshot`. V produkci ale existovala jako `SECURITY DEFINER` s `EXECUTE` pro `authenticated` (pracuje jen s vlastní hodinou volajícího) a její větev `start` nastavovala `timer_remaining_seconds = NULL` i pro první timer blok (chyba ze živé hodiny 24. 9. se v běžném provozu projevila přes Worker a opravil ji Worker 0.8.16 v 0.9.120). Ověřeno na dočasné Neon větvi (poté smazané): migrace projde i opakovaně, snapshotová reconcile zůstává. Po aplikaci je potřeba obnovit schema cache Data API. `LIVE_RESILIENCE.md` funkci zatím uvádí (řeší audit C2).

Aktualizováno: 2026-09-25 — interní verze **0.9.157**: ověření přihlášení na Neonu už při chybě Neon Auth nepřijímá starší ověření session, pokud jde o odpověď typu 401/403 (audit B4). `lib/neon/request-client.ts` dosud při jakékoli chybě `getSession()` použil až 5 min staré úspěšné ověření a při chybě `token()` vrátil JWT z cache až do jeho vypršení. Nově to platí jen pro skutečný výpadek nebo omezení služby: status 5xx (knihovna hlásí chybu spojení jako 502) nebo 429 (`lib/neon/auth-failure.ts`, `isTransientNeonAuthFailure`). Jakýkoli jiný status, například 401 nebo 403, je odpověď o session: přihlášení se odmítne a ověření v cache se smaže. Chyba bez statusu se bere jako trvalá. Odvolaná session vrací z knihovny `data: null` bez chyby a odmítala se už dřív. Zůstává 60s cache úspěšného ověření a sdílení souběžných kontrol z #292, které řeší původní sporadické 401 při pollingu. `verify-neon-auth-session-binding` testuje rozhodovací funkci (5xx/429 ano, 400/401/403/404 a chyba bez statusu ne) a obě místa použití. Veřejná verze: ne (bezpečnostní oprava).

Aktualizováno: 2026-09-25 — interní verze **0.9.156**: cron routy ověřují `CRON_SECRET` v konstantním čase (audit B3). Nový `lib/cron-auth.ts` (`isAuthorizedCronRequest`) zahashuje očekávanou i přijatou hlavičku `Authorization: Bearer …` přes SHA-256 a porovná je `timingSafeEqual`; bez `CRON_SECRET` odmítá. Používají ho všechny čtyři crony (`complaints`, `neon-grading`, `organization-billing`, `service-changes`) místo dřívějšího `!==`. `verify-neon-migration-preparation` hlídá, že každá route v `app/api/cron` helper používá a sama `CRON_SECRET` nečte; `verify-complaints` a `verify-school-organizations` kontrolují nové volání. Chování pro Vercel Cron se nemění. Veřejná verze: ne (infrastruktura).

Aktualizováno: 2026-09-25 — **LIVE ceny balíčků návrhů hodnocení od AI** (bez změny verze): Václav spustil `scripts/create-ai-grading-topup-prices.mjs --mode live --confirm-live` s jednorázovým restricted klíčem (Products a Prices: Write) z hlavního živého účtu Syllonaut. Suchý běh převzal z ceny Teacher Pro `tax_behavior` `unspecified` a `tax_code` `txcd_10103001` jako v sandboxu. Vznikly 3 produkty (`syllonaut_ai_grading_60/100/200`) a 9 cen: CZK `price_1UJTrkArYiDuwS3OsW9q8Kkf`, `price_1UJTrmArYiDuwS3OyNhb4wNZ`, `price_1UJTrnArYiDuwS3OZUQZBMwu`; EUR `price_1UJTrnArYiDuwS3OXluHAG0D`, `price_1UJTroArYiDuwS3OPZioCMEU`, `price_1UJTrpArYiDuwS3O6M6RHfmr`; USD `price_1UJTrqArYiDuwS3OBQhTMYMm`, `price_1UJTrqArYiDuwS3OstArRoXq`, `price_1UJTrrArYiDuwS3O6ogXIR1P` (pořadí 60 / 100 / 200). Produkční `private.billing_topup_prices` obsahuje všech 9 jako `livemode = true`, `active = true` (ověřeno čtením). Nákup zůstává vypnutý; zbývají Stripe eventy `checkout.session.completed` a `checkout.session.async_payment_succeeded`, zapnutí `AI_GRADING_TOPUPS_ENABLED` a blok v Resend šabloně. Restricted klíč ve Stripe po běhu smazat.

Aktualizováno: 2026-09-25 — **produkční ověření 0.9.155** (#389, `a4c3dde`, bez změny verze). `www.syllonaut.com` obsluhuje nasazení `dpl_3Jj3BqBXXBJbaq6yQDzxp5eFM1RL` (Ready). `/cs/terms` ukazuje „Verze 1.12 · účinná od 25. 9. 2026“ a článek „5a. Dokupované balíčky návrhů hodnocení od AI“, `/en/terms` „Version 1.12 · effective 25 September 2026“ a „5a. Additional AI grading suggestion packs“; text VOP 1.11 se už nezobrazuje. Nákup balíčků zůstává vypnutý (`POST /api/billing/stripe/topup/checkout` vrací 404, Ceník balíčky nezmiňuje). Žádná odpověď 500 po nasazení.

Aktualizováno: 2026-09-25 — **Stripe LIVE klíč a školní platba kartou** (bez změny verze). Kontrola po 0.9.153 ukázala, že jediný LIVE klíč aplikace „Syllonaut Live“ (`rk_live_…WW0G`, omezený) měl u Customers jen Read. Školní platba kartou ale nejdřív zakládá zákazníka ve Stripe (`POST /v1/customers`, `lib/organization-payment.ts`), takže přechod na platbu by selhal. Individuální Checkout tím dotčen nebyl, protože zákazníka tam vytváří Stripe Checkout sám. Vlastník 2026-09-25 přidal **Customers: Write**, **Charges and Refunds: Write** a **Payment Intents: Read**. Bez Charges and Refunds by webhook při každé refundaci selhal na `GET /v1/charges/:id` (`lib/stripe-refunds.ts`) a `GET /v1/refunds/:id`, a odstoupení (LEGAL-008, `lib/stripe-withdrawal.ts`) by nevytvořilo refundaci ani nenačetlo Payment Intent. Klíč má nyní 7 oprávnění: Charges and Refunds Write, Checkout Sessions Write, Customer Portal Write, Customers Write, Invoices Read (pokrývá i `invoice_payments`), Payment Intents Read, Subscriptions Write (pokrývá i `subscription_schedules`). Ověřeno v dashboardu; hodnota ve Vercelu se neměnila. Prices nejsou potřeba (`retrieveStripePrice` nikdo nevolá). Ověřeno čtením: LIVE webhook `we_1UHHNRArYiDuwS3OzTA8LueC` je aktivní a odebírá všech 16 zpracovávaných událostí včetně `invoice.paid` (aktivace školy). Produkční `billing_prices` má 18 aktivních LIVE cen Team/School/Campus (měsíčně i ročně, CZK/EUR/USD). V Production jsou všechny proměnné `SYLLONAUT_INVOICE_*`, `RESEND_BANK_WEBHOOK_SECRET` a `BANK_NOTIFICATION_*`. **Test bez platby 2026-09-25:** (1) vlastník na účtu bez role admin stáhl na `/school` nabídku PDF (oprava 0.9.153 funguje) a prošel ostrou objednávku School/ročně/CZK kartou až na platební stránku Stripe, kterou nezaplatil. V DB vznikla organizace a objednávka `awaiting_payment` s LIVE zákazníkem `cus_…` a Checkout Session `cs_live_…`; v produkčních logech žádná chyba. Tím je ověřen živý klíč (Customers Write), živá cena i Checkout. (2) Na dočasné Neon větvi (poté smazané) se pro tuto objednávku zavolaly `sync_organization_invoice_event` a `sync_organization_invoice_payment_event_v2` stejně jako webhook po `invoice.paid`: organizace `active` (`automatic_card`, roční období), objednávka `paid`, platba spárovaná, opakovaná událost odmítnutá jako `duplicate_event`, vlastníkovi se zapnuly školní entitlementy. (3) Úklid v produkci: testovací organizace smazána jedním chráněným `DELETE` (jen `awaiting_payment`, ne `is_internal_test`; kaskádou objednávka a členství, předem vyzkoušeno na dočasné větvi); `organization_orders` je opět prázdná, testovací účet je zpět na Free bez školních entitlementů. Ve Stripe zůstal jen nepoužitý LIVE zákazník, Checkout Session vyprší sama. **Otevřené:** skutečná platba (doručení `invoice.paid` ze Stripe s metadaty organizace) a nákup fakturou včetně spárování avíza Air Bank zatím neproběhly.

Aktualizováno: 2026-09-25 — interní verze **0.9.155**: **VOP 1.12** (`2026-09-25-v13`, účinné od 25. 9. 2026) uzavírá **LEGAL-023**. Právník schválil článek 5a „Dokupované balíčky návrhů hodnocení od AI“ a oba souhlasy u objednávky balíčku beze změny (`TERMS_AI_GRADING_TOPUP_CLAUSE`, `AI_GRADING_TOPUP_CONSENT` v `lib/terms-content.ts`); vlastník projektu potvrdil účinnost od 25. 9. 2026 a rozhodl, že **stávající souhlasy v4–v12 zůstávají dostatečné** pro běžné používání (článek 5a se týká jen dobrovolného nákupu balíčku, kde se VOP 1.12 odsouhlasí v checkoutu; nové registrace a objednávky evidují 1.12). Změny: `lib/legal.ts` (`TERMS_VERSION = '1.12'`, `TERMS_EFFECTIVE_DATE = '2026-09-25'`, `TERMS_ACCEPTANCE_KEY = '2026-09-25-v13'`, `'2026-09-24-v12'` v `TERMS_PRODUCT_ACCESS_KEYS`, `TERMS_AI_GRADING_TOPUP_ARTICLE_ACTIVE = true`), datum účinnosti na `/terms`, `/terms/accept` a ve smluvním snapshotu, snapshot předplatného i balíčku je nově pevně svázaný s VOP 1.12 a obsahuje článek 5a mezi články 5 a 6. Na Neonu se souhlasy zapisují přímo do `private.terms_acceptance_events` a přístup se ověřuje podle `TERMS_PRODUCT_ACCESS_KEYS`, takže nový klíč nepotřebuje DB migraci. Nákup balíčků zůstává vypnutý (`AI_GRADING_TOPUPS_ENABLED`); zbývají LIVE ceny, Stripe eventy, zapnutí vypínače a blok v Resend šabloně. Kontroly: `verify-terms`, `verify-provider-contact` a `verify-ai-grading-topups` očekávají 1.12; snapshoty předplatného i balíčku (CS/EN) vyrenderované lokálně; `npx tsc --noEmit`, `npm run check`, `npm run check:accessibility`, `npx next build`. Veřejně zobrazovaná verze zůstává 0.9.140.

Aktualizováno: 2026-09-25 — **produkční ověření 0.9.154 / SEC-018** (#387, `46ad360`, bez změny verze). `www.syllonaut.com` obsluhuje nasazení `dpl_89587H2MuZZ9mdYWdHdLTtRXHgXi` (Ready) sestavené z `46ad360`. Bez přihlášení: `/cs` a `/api/auth/get-session` 200; požadavek se dvěma cookies `__Secure-neon-auth.session_token` dostal 401 a `Set-Cookie` mazající kopii `Domain=www.syllonaut.com`; 5 minut po nasazení žádná odpověď 500. Vlastník projektu v prohlížeči, kde se chyba projevila, ověřil přepnutí účtů (přihlášení `vaclav.loubek@gmail.com` → odhlášení → přihlášení `loubek@icloud.com`): zobrazuje se správný účet.

Aktualizováno: 2026-09-25 — interní verze **0.9.154** / **SEC-018** (bezpečnostní oprava přihlášení): po přihlášení účtem `loubek@icloud.com` byl vlastník v tom samém prohlížeči přihlášený jako `vaclav.loubek@gmail.com`. **Data účtů jsou v pořádku** (v `neon_auth.user`/`account` dva oddělené účty, každý s vlastním heslem); chyba byla v cookies relace Neon Auth, cizí zařízení se k účtu takto dostat nemohlo, ale na sdíleném (školním) počítači ano. Příčiny: (1) `/api/auth/[...path]` nastavoval cookies s `Domain=<host>`, server actions přihlášení/odhlášení host-only → prohlížeč držel dvě cookies `__Secure-neon-auth.session_token` / `.local.session_data` se stejným jménem a odhlášení smazalo jen jednu; (2) naše cache relace v `lib/neon/request-client.ts` brala **první** session cookie z hlavičky, knihovna **poslední**; (3) knihovna `@neondatabase/auth` (0.5.0-beta) vrací `getSession()` z podepsané kopie `session_data` bez kontroly, že patří k odeslané session cookie; když se po přihlášení nepodaří kopii obnovit, až 5 minut pojmenuje předchozí účet. Oprava: nový `lib/neon/auth-cookies.ts`; auth proxy přepisuje cookies na host-only (`withHostOnlyNeonAuthCookies`) a na dvojici session cookies odpoví 401; `proxy.ts` při duplicitě vyexpiruje starou `Domain=` kopii; `getVerifiedNeonSession()` čte hlavičku Cookie stejně jako knihovna, dvojici cookies bere jako odhlášení, kopii `session_data` použije jen se shodným `session.token`, jinak ověří relaci u Neon Auth (`disableCookieCache`); `getNeonAppUser` jde přes ni; přihlášení i ověření kódem nejdřív smažou starou kopii `session_data`. Ověřeno reprodukcí se skutečnou knihovnou (podvržené `next/headers` a upstream): původní kód pro iCloud session cookie se starou Gmail kopií vrátil Gmail, opravený vrací iCloud, dvojici cookies odmítne a běžné přihlášení dál nevolá Neon Auth navíc. Nový `scripts/verify-neon-auth-session-binding.mjs` v `npm run check`; `npx tsc --noEmit`, `npm run check`, `npx next build`. Po nasazení: kdo měl obě kopie cookies, bude jednou odhlášen (jeden požadavek), pak se přihlásí normálně. Přepnutí dvou účtů v produkci ověřeno (viz záznam výše). Veřejně zobrazovaná verze: ne (bezpečnostní oprava bez změny UI).

Aktualizováno: 2026-09-25 — interní verze **0.9.153**: krok nabídky školního tarifu (`POST /api/organizations/quote`) používá sdílený přepínač `isPublicSchoolBillingEnabled()` jako ostatní školní routy (audit B2, rozhodnutí vlastníka: školám se prodává). Route dosud vyžadovala starou proměnnou `STRIPE_LIVE_SCHOOL_BILLING_PUBLIC_ENABLED=true`, která v Production chybí (vlastník ověřil ve Vercelu 2026-09-25). Běžný uživatel proto na `/school` po kliknutí na volitelné tlačítko „Stáhnout cenovou nabídku PDF“ dostal 403 `school_live_billing_not_public` (admin měl výjimku). Samotná objednávka (`POST /api/organizations`) a platba kartou či fakturou tím zablokované nebyly, ty už sdílený přepínač používaly. Nouzové vypnutí zůstává jediné: `STRIPE_LIVE_SCHOOL_BILLING_EMERGENCY_DISABLED=true` (i v `.env.example`, stará proměnná odstraněna). `scripts/verify-school-organizations-v11.mjs` nově hlídá, že `/quote` používá sdílený přepínač a nečte starou proměnnou. Opravena místa v PROJECT.md, která tvrdila, že se Team / School / Campus zatím neprodávají. Veřejná verze: ne (platby a fakturace).

Aktualizováno: 2026-09-25 — **produkční ověření 0.9.152** (#375, `2cd1c37`, bez změny verze). `www.syllonaut.com` obsluhuje nasazení `dpl_Hc3LgBPHcafbpNcxvaLnA9AGHrpa` (Ready) sestavené z `2cd1c37`. Vypínač `AI_GRADING_TOPUPS_ENABLED` je v Production vypnutý: `POST /api/billing/stripe/topup/checkout` i `POST /api/sessions/<id>/evaluations/request-ai-suggestions` vrací 404, `/cs/pricing` ani `/en/pricing` balíčky nezmiňují, `/cs/terms` je dál VOP 1.11 bez článku 5a. Migrace 0019 je v produkci (aplikovaná před sloučením) a `get_ai_quota()` vrací `grading_credit_remaining` a `grading_credit_next_expiry`. Žádná odpověď 500 po nasazení. Do spuštění zbývá: právní posouzení LEGAL-023, aktivace VOP 1.12, LIVE ceny (příkaz v Otevřených provozních bodech, spouští Václav), eventy `checkout.session.completed` a `checkout.session.async_payment_succeeded` ve Stripe event destination, zapnutí vypínače a blok s balíčky v Resend šabloně (Cowork). Dočasná Neon větev `tmp/0017-ai-grading-topups` (`br-proud-bird-b27dr1ix`) s testovacími daty je po ověření smazaná (2026-09-25, na pokyn vlastníka projektu).
...
Aktualizováno: 2026-09-25 — **produkční ověření 0.9.151** (#381, `c1c1cd8`, bez změny verze). Produkce obsluhuje nasazení `dpl_Dh4PK2pbxWcoZ5eDBNBtmgMvD7LF` (Ready) z `c1c1cd8`. `/cs/dpa` a `/en/dpa` ukazují DPA 1.4 s Nápovědou Syllonautu v sekci 2 i u dalšího zpracovatele OpenAI. Tím je Nápověda kompletně nasazená: kód 0.9.144–0.9.145, zpřístupnění Teacher, Teacher Pro, School a Campus 0.9.150 (migrace 0018, Privacy Notice 1.9), DPA 1.4 v 0.9.151. Otevřené: ověřit Nápovědu z pohledu skutečného učitele s placeným tarifem (zatím jen admin) a hlídat útratu klíče AI Gateway `syllonaut-help` (limit $5/měsíc) a záznamy `help_assistant_requests`.
...
**Aktuální produktová verze: 0.9.170** — student si během živé hodiny může pod aktuálním úkolem otevřít **„Předchozí aktivity“**: už proběhlé aktivity a svou vlastní (nebo týmovou) odpověď na ně, jen pro čtení (0.9.170). Předchozí stav: přihlášení učitelé s tarifem Teacher, Teacher Pro, School nebo Campus mají **Nápovědu Syllonautu**, AI asistenta k ovládání aplikace, tarifům a limitům (0.9.150). Předchozí stav: Syllonaut má české a anglické UI, regionální výchozí volbu jazyka a oddělený jazyk generované lekce. **Sdílení lekcí je produkčně dokončené a E2E ověřené:** autor vytváří odvolatelný read-only snapshot, příjemce musí pro uložení a spuštění použít vlastní účet a dostane samostatnou kopii. Share link je záměrně přenositelný a počítá se s ním i pro veřejné ukázkové lekce a akviziční distribuci. Free účet generuje nové lekce pouze v aktivním jazyce UI a při AI revizích nesmí změnit hlavní jazyk existující lekce nebo bloku. Teacher, Teacher Pro a budoucí Team/School/Campus mají benefit **Lekce v libovolném jazyce**, včetně automatické detekce jazyka zadání, explicitní volby dalšího jazyka a změny jazyka při AI revizi. Entitlement je vynucený serverově.

### Předchozí aktivity, projekce stavu pro studenta a navazování bloků 0.9.170 — 2026-09-26

- **Rozhodnutí: projekce stavu Live Control pro studenta.** Worker dosud vracel studentské capability celý snapshot (odpovědi a jména všech účastníků, všechny bloky včetně budoucích) a přes WebSocket celé eventy; klient cizí data filtroval až v prohlížeči. Od Workeru **0.8.17** (`cloudflare/live-control/src/index.ts`, `projectSnapshotForStudent`) dostane každá role kromě `teacher` a `presenter` na `GET /state` jen vlastní řádek účastníka a vlastní odpovědi, odpověď vlastního týmu bez ID spoluhráčů, bloky do aktivního včetně (v lobby žádné), `lessonSnapshot.totalBlocks`, `teams[].memberCount` spočítané na serveru a prázdné `events`. Studentské sockety dostanou jen `{ type: 'wake', revision }` a klient si stav znovu načte. Opakované `operationId` jiného aktéra už nevrací jeho event. Učitel a presenter dostávají data beze změny. `protocolVersion` zůstává **2**. Nový klient používá `memberCount` a `totalBlocks`, pokud existují, a funguje i se starým Workerem. Starý klient s novým Workerem funguje (jméno, můj tým, aktivní blok, index, stav odpovědi), jen v záložní cestě ukáže počítadlo „N / N“ a velikost týmů podle sebe. Proto se nasazuje **nejdřív Vercel, potom Worker**.
- **Presenter token:** mintuje ho jen `GET /api/sessions/[id]/live-control?role=presenter` pro přihlášeného vlastníka hodiny (`teacher_id = userId`), nebo z jeho podepsaného httpOnly resume cookie. Nikdo jiný ho nedostane. Presenter dostává plný snapshot, ale v UI z něj ukazuje jen počty.
- **Pravidlo navazování v promptu** (`baseRules` v `lib/ai.ts`, tedy generování i obě AI revize): navazování aktivit je žádoucí. Když blok pracuje s obsahem předchozí aktivity, stručně ho zopakuje v `instructions`/`dataTable`. Na vlastní dřívější odpověď studenta nebo výstup týmu odkazuje číslem i názvem aktivity (např. „aktivita 3 ‚Hlavní příčiny‘“). U výsledku jen ze třídy ho popíše, případně s pokynem v `teacherNote`. Nikdy nepíše jen „výše“ nebo „předchozí úkol“. Číslo je pořadí bloku v lekci včetně intro, reveal, poll a timer, stejně jako počítadlo studenta. Revize udrží odkazy aktuální. **AI úprava jedné aktivity** (`reviseBlock`) nově dostává číslovaný přehled typů a názvů všech bloků s vyznačeným upravovaným blokem (dosud znala jen název lekce, cílovou skupinu a cíle).
- **„Předchozí aktivity“:** `state()` v obou implementacích (`lib/neon/student-session-server.ts`, `lib/student-session-server.ts`) při `status = 'live'` vrací `previousBlocks`: bloky s indexem menším než aktivní, přes `publicBlock` (bez `teacherNote`, `correctAnswer`, `gradingRubric`, `modelAnswer`), `myAnswer` (odeslaná, jinak poslední uložená) a u `team_task` `myTeamAnswer` vlastního týmu. Načítá se jedním dotazem `block_id IN (…)` na tabulku; při chybě čtení se přehled ukáže bez odpovědí a živý pohled nespadne. Záložní cesta skládá totéž z projekce Workeru a offline cache ho ukládá jako součást stavu. UI (`components/StudentPreviousActivities.tsx`) je pod aktuálním úkolem: sbalená sekce „Předchozí aktivity (n)“ / „Previous activities (n)“, v ní sbalitelné položky „č. Název“ (číslování jako „3 / 8“), zadání přes `LiveBlock` a „Tvoje odpověď“ / „Tvoje týmová odpověď“ / „Bez odpovědi“. Používá nativní `<details>`/`<summary>`, `lang`/`dir` podle jazyka lekce a nemá žádné vstupní pole ani tlačítko. Stav otevření drží React, takže změna aktivního bloku sekci nezavře a nepřesune fokus. Po „Předchozí“ se seznam zkrátí. Privacy Notice ani DPA se nemění: student vidí jen svá vlastní data, projekce zpracování ve Workeru nerozšiřuje.
- **Kontroly:** nový `scripts/verify-live-student-projection.mjs` (v `npm run check`) spouští skutečný Worker proti `node:sqlite` a falešným socketům. Student přes `/state`, WebSocket ani vstupní Worker s podvrženými hlavičkami nedostane cizí odpovědi, jména, ID, events ani budoucí bloky; učitel a presenter dostanou plný snapshot. Dále spouští skutečné `state()` obou backendů nad tabulkami v paměti: jen dřívější bloky, bez skrytých polí a cizích odpovědí, jeden dotaz na tabulku. `verify-revision-references` hlídá pravidlo ve všech třech system promptech, že navazování nezakazuje, a přehled bloků v revizi jedné aktivity. Prošly také `tsc`, `npm run check`, `npm run check:accessibility`, `next build` a `wrangler deploy --dry-run`. UI je ověřené v lokálním harnessu (375 px, bez horizontálního posunu, zachování otevřených položek a fokusu při změně bloku). Placené AI cally se nespouštěly. Ruční test proběhl 26. 9. v produkci po nasazení Workeru (dva studenti, týmová lekce, `/state` a WebSocket), viz záznam „produkční nasazení 0.9.170 a Workeru 0.8.17“.
- **Nasazení Workeru 0.8.17:** 26. 9. 2026 po produkčním nasazení 0.9.170 na Vercelu, ve chvíli bez živé hodiny, `npx wrangler deploy` z `origin/main` `22948d6`. Version ID `46616e06-83d9-4722-8938-ed0202ce1d9e`, `/health` vrací `workerVersion=0.8.17`.
...
- od **0.9.174** se během živé hodiny **aktivní kvíz do skóre ani pořadí nezapočítá, dokud nejsou zveřejněné jeho výsledky** (Neon migrace **0025**, `lib/scoreboard-server.ts`); po přechodu učitele dál a po skončení hodiny se počítá;
...
- **[LEGAL-024 — RESOLVED 0.9.164, bez posouzení právníkem] Plná refundace ukončí předplatné.** VOP 1.13 (`2026-09-26-v14`, účinné od 26. 9. 2026) doplňuje článek 6: vrátí-li Syllonaut na žádost uživatele celou platbu za aktuální období individuálního předplatného, předplatné končí ke dni vrácení a účet přejde na Free; neplatí pro vrácení duplicitní platby. Odpovídá chování od 0.9.162. Vlastník 26. 9. rozhodl: změna je neutrální (uživatel dostane zpět celou platbu), souhlasy v4–v13 zůstávají dostatečné, oznámení podle čl. 10 se neposílá. Při příští konzultaci dát právníkovi k potvrzení.
- **[LEGAL-023 — RESOLVED 0.9.155] Balíčky návrhů hodnocení od AI.** Právník 25. 9. 2026 schválil článek 5a VOP a oba souhlasy u objednávky balíčku beze změny (podklad a otázky v `docs/AI_GRADING_TOPUPS_LEGAL_REVIEW.md`). VOP 1.12 (`2026-09-25-v13`) je účinné od 25. 9. 2026; stávající souhlasy v4–v12 zůstávají dostatečné (rozhodnutí vlastníka projektu). Nákup balíčků zůstává za vypínačem `AI_GRADING_TOPUPS_ENABLED`, dokud nejsou LIVE ceny a Stripe eventy.
...
- aktuální veřejně viditelnou verzi určuje `APP_VERSION` v `lib/version.ts` spolu s řádkem „Aktuální produktová verze“ (k 2026-09-26 `0.9.170`); interní revize ji na dashboardu nemění;
...
Roční varianta komunikuje přibližně **2 měsíce zdarma**. Individuální tarify **Teacher / Teacher Pro jsou produkčně veřejně prodejné přes LIVE Stripe Checkout**; Free CTA vede na registraci bez platební karty. Team / School / Campus jsou od 2026-09-20 (`d13a5f2`) veřejně samoobslužně prodejné; nouzové vypnutí pro všechny kromě admina je `STRIPE_LIVE_SCHOOL_BILLING_EMERGENCY_DISABLED=true` (`lib/school-billing-launch.ts`).
...
9. veřejný self-service školní billing je zapnutý od 2026-09-20 (`d13a5f2`). Starou proměnnou `STRIPE_LIVE_SCHOOL_BILLING_PUBLIC_ENABLED` od 0.9.153 nečte žádný kód; vypnutí jen přes `STRIPE_LIVE_SCHOOL_BILLING_EMERGENCY_DISABLED=true`.
...
- admin: entitlement automaticky;
- Team / School / Campus mají multilingual benefit serverově napojený přes implementovanou organizační vrstvu; veřejný self-service školní billing je spuštěný;
...
- **SEC-018** — záměna účtu po přihlášení ve stejném prohlížeči (0.9.154) → Neon Auth cookies jen host-only, duplicitní session cookies = odhlášení a úklid staré `Domain=` kopie, `session_data` kopie platí jen se shodným session tokenem, přihlášení maže starou kopii; regresní `verify-neon-auth-session-binding`;
...
**Veřejný Ceník je dokončen; individuální Teacher / Teacher Pro LIVE billing je produkčně spuštěný a end-to-end ověřený. Team / School / Campus mají organizační a billingovou vrstvu a jsou veřejně samoobslužně prodejné.**
...
- Free signup CTA; Teacher / Teacher Pro mají aktivní LIVE Checkout CTA; Team / School / Campus mají veřejný self-service nákup;
