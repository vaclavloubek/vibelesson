...
Aktualizováno: 2026-09-19 pro interní verzi 0.9.23 — hotfix tisku pracovních listů opravuje prázdný Safari/PDF výstup a stránkování aktivit: print CSS explicitně zachovává worksheet root a každé cvičení se při tisku chová jako nedělitelný stránkový blok, takže nezačne na konci stránky a nepokračuje na další, pokud se samo vejde na A4. Veřejně zobrazovaná verze na dashboardu zůstává 0.9.20.
...
- **0.9.23 interní hotfix** — tisk/PDF pracovních listů: odstraněn křehký print selector `body > :not(#main-content)`, který mohl v Safari skrýt celý worksheet a vytvořit prázdné PDF. Print režim nyní explicitně zachovává `#main-content`, skrývá pouze okolní aplikační chrome a převádí seznam aktivit z CSS Gridu na běžný tiskový flow. Každá aktivita má současně moderní i legacy zákaz page-breaku (`break-inside: avoid-page` + `page-break-inside: avoid`), aby cvičení nezačínalo na konci jedné stránky a nepokračovalo na další, pokud se samo vejde na A4. Veřejně zobrazovaná verze zůstává 0.9.20.
