...
**Live billing acceptance je od 2026-09-19 kompletně ověřený:** routing nedůvěřuje pouze předem zvolené zemi. Live webhook před provisioningem načte dokončený Checkout Session a ověří skutečnou billing country proti měně a Merchant-of-Record režimu. Země v jiné zemi stejné routing větve (např. DE → FR v EUR + Managed Payments) je přípustná a do DB se uloží skutečná země; přesun do jiné větve (např. DE → US) fail-closed odmítne provisioning. Ověřena byla skutečná CZ platba 199 Kč přes standard Stripe, live Customer/subscription provisioning, cancellation webhook, admin entitlement preservation, Customer Portal přes aplikaci a samostatný ne-admin přechod Free → Teacher → Free. Běžný Teacher účet dostal přesně 25 lekcí, 100 AI úprav a multilingual entitlement, zatímco AI grading a složky zůstaly vypnuté. Obě testovací subscription byly zrušené a oba plné refundy 199 Kč Stripe eviduje jako `succeeded`. Veřejná placená CTA zůstávají vypnutá pouze do vědomého rozhodnutí spustit ostrý prodej.
...
Security audit SEC-001 až SEC-016 je dispositioned. Accessibility technický baseline je implementovaný a nasazený. GDPR/cookies/privacy baseline je dokončený. GA4 je produkčně aktivní při opt-in. **Stripe sandbox lifecycle i LIVE acceptance individuálních plánů jsou dokončené a E2E ověřené.** Live ověření zahrnuje skutečnou CZ platbu, actual billing-country guard, provisioning, Customer Portal, admin preservation, ne-admin Free → Teacher → Free, cancellation webhooky a refundy. Ostrý prodej zůstává zatím vypnutý pouze jako produktový launch gate; technický acceptance pro Teacher / Teacher Pro je splněný. Aktuální produktová verze je 0.9.17; uvnitř ní zůstává zachovaný live hardening baseline 0.8.16 / Worker 0.8.14 protocol 2.
...
6. **live billing acceptance dokončen**: skutečná CZ platba, live Customer/subscription provisioning, actual billing-country kontrola, Customer Portal, admin preservation, ne-admin Free → Teacher → Free, cancellation webhooky i plné refundy jsou ověřené. Další krok už není test infrastruktury, ale vědomé produktové rozhodnutí zapnout veřejné CTA pro Teacher / Teacher Pro; Team / School / Campus zůstávají mimo live billing;
