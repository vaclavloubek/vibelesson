...
Aktualizováno: 2026-09-20 — interní i veřejná verze **0.9.80** doplňuje produkční právní a objednávkový baseline: bilingvní Obchodní podmínky, povinný souhlas při registraci, samostatný právní souhlas před individuálním LIVE checkoutem včetně okamžitého zpřístupnění digitální služby, souhlas a potvrzení oprávnění u školních objednávek, auditní stopu v Stripe metadata / organization billing snapshotu a potvrzení souhlasu v aktivačním e-mailu. Jde o větší produktový milník, proto se podle verzovacího pravidla posouvá i veřejně zobrazovaná verze z 0.9.30 na 0.9.80.

**Aktuální produktová verze: 0.9.80** — Syllonaut má české a anglické UI, regionální výchozí volbu jazyka a oddělený jazyk generované lekce. **Sdílení lekcí je produkčně dokončené a E2E ověřené:** autor vytváří odvolatelný read-only snapshot, příjemce musí pro uložení a spuštění použít vlastní účet a dostane samostatnou kopii. Share link je záměrně přenositelný a počítá se s ním i pro veřejné ukázkové lekce a akviziční distribuci. Free účet generuje nové lekce pouze v aktivním jazyce UI a při AI revizích nesmí změnit hlavní jazyk existující lekce nebo bloku. Teacher, Teacher Pro a Team/School/Campus mají benefit **Lekce v libovolném jazyce**, včetně automatické detekce jazyka zadání, explicitní volby dalšího jazyka a změny jazyka při AI revizi. Entitlement je vynucený serverově.

### Obchodní podmínky a právní acceptance 0.9.80 — 2026-09-20

- přidána bilingvní stránka **Obchodní podmínky / Terms of Service** na `/cs/terms` a `/en/terms`; přímé `/terms` se lokalizuje stejně jako Pricing/GDPR;
- podmínky pokrývají identitu poskytovatele, registraci, Free a placené tarify, Team/School/Campus, platby kartou i vlastní bankovní fakturou, automatické obnovování, odstoupení spotřebitele, digitální plnění, reklamace, AI výstupy, uživatelský obsah, školy/studenty, dostupnost služby, pozastavení, odpovědnost a ADR přes ČOI;
- společná patička nově odkazuje na Obchodní podmínky vedle GDPR a nastavení cookies;
- registrace má povinný, předem nezaškrtnutý souhlas s aktuální verzí podmínek; marketingový souhlas zůstává samostatný a dobrovolný; do Supabase Auth metadata se ukládá `terms_accepted=true` a `terms_version=1.0`;
- individuální LIVE Checkout Teacher/Teacher Pro vyžaduje samostatně **souhlas s podmínkami + potvrzení automatického obnovování** a **výslovnou žádost o okamžité zpřístupnění digitální služby před uplynutím 14 dnů**; API požadavek bez aktuální verze a obou potvrzení failuje uzavřeně;
- verze podmínek, serverový čas souhlasu a immediate-access flag se zapisují do Stripe Checkout i Subscription metadata; aktivační transakční e-mail tento souhlas potvrzuje a současně byly opraveny jeho dříve zastaralé Teacher/Teacher Pro AI limity;
- vytvoření Team/School/Campus objednávky vyžaduje souhlas s podmínkami a potvrzení, že objednávající je oprávněn jednat za organizaci; card CTA výslovně používá **„Objednávka zavazující k platbě“** a invoice CTA stejně jasně komunikuje platební povinnost;
- školní API stejné podmínky vynucuje serverově; verze, serverový čas, uživatel a potvrzení oprávnění se zapisují do existujícího `organization_orders.billing_snapshot.legalAcceptance`, takže nebyla nutná nová DB migrace;
- GDPR stránka byla srovnána se současným stavem: doplněno IČO a aktuální PSČ/adresa a odstraněna zastaralá formulace, že školní účty jsou teprve budoucí;
- regresní kontrakt: nový `scripts/verify-terms.mjs` + rozšíření Stripe checkout a billing-lifecycle testů;
- veřejně zobrazovaná verze dashboardu se jako součást tohoto většího komerčního/právního milníku posouvá na **0.9.80**.
...
6. **live billing je veřejný a lifecycle e-maily mají produkční E2E acceptance COMPLETE / PASS**; správa předplatného po 0.9.23/0.9.25 načítá produkční stav správně a admin UX je ručně ověřený PASS. LIVE restricted key permissions byly doplněny a read cesta je produkčně ověřená. Další billing acceptance krok je první skutečná změna tarifu, která ověří write/schedule cestu; změna země/měny zůstává řízená. Team / School / Campus jsou veřejně zapnuté; další krok je sledovat první skutečné objednávky a případné billing edge-cases;
...
9. dokončit **V1.1 školní billing acceptance** bez opakování již uzavřených V1 testů: finální card Checkout E2E, owner pohled po bankovní úhradě, bankovní renewal, případně Stripe cancel/restore; potom cleanup sandbox `Test School`, vrácení `loubek@icloud.com` na `user/free` a návrat do interní `Testovací školy`; veřejný Team/School/Campus self-service je již spuštěný. Automatický `bank_match` backend je připravený, ale Air Bank/open-banking provider zatím není připojený;
10. před veřejným prohlášením WCAG 2.2 AA provést manuální WCAG-EM evaluaci podle `ACCESSIBILITY.md`;
11. po úspěšném reálném pondělním acceptance testu 2026-09-21 znovu rozhodnout, zda je 0.9.80 poslední pre-1.0 milestone a zda produkt povýšit na 1.0.0; konečné rozhodnutí podle verzovacího pravidla zůstává vlastníkovi projektu.
