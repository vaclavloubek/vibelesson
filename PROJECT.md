...
**Live podmínka od 0.9.13 implementována a 2026-09-19 poprvé ostrým testem ověřena:** routing nedůvěřuje pouze předem zvolené zemi. Live webhook před provisioningem načte dokončený Checkout Session a ověří skutečnou billing country proti měně a Merchant-of-Record režimu. Země v jiné zemi stejné routing větve (např. DE → FR v EUR + Managed Payments) je přípustná a do DB se uloží skutečná země; přesun do jiné větve (např. DE → US) fail-closed odmítne provisioning. První skutečná platba 199 Kč v CZK prošla jako CZ + standard Stripe; po hotfixu 0.9.15 se správně zapsal live Customer, active Teacher monthly subscription i CZ billing country do Supabase, admin účet zůstal admin. Subscription byla následně zrušena, `customer.subscription.deleted` se propsal do Supabase a plný refund 199 Kč byl ve Stripe úspěšně vytvořen. Zbývá live Customer Portal acceptance a samostatný ne-admin test, že běžný Free profil po platbě dostane Teacher entitlementy; veřejná placená CTA zůstávají vypnutá.
...
- **0.9.16 / SEC-017** — serverový guard na `PUT /api/lessons/[id]`: Free účet nesmí přes replacement payload změnit `lesson.language`; guard používá autoritativní DB lekci + serverový profil a je krytý regresním testem `verify-lesson-replacement-entitlement.mjs`
...
6. dokončit **live billing acceptance**: první skutečná CZ platba, live Customer/subscription provisioning, skutečná billing-country kontrola, cancellation webhook a refund jsou ověřené; zbývá projít live Customer Portal přes aplikaci a samostatně otestovat ne-admin Free → Teacher entitlement provisioning. Veřejná placená CTA zapnout až po těchto dvou acceptance krocích;
