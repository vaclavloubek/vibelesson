...
Aktualizováno: 2026-09-19 pro interní verzi 0.9.22 — hotfix samoobslužné správy předplatného po prvním produkčním načtení: odstraněna zbytečná závislost renderu na Stripe Prices API, ceny Teacher / Teacher Pro mají jeden sdílený katalog s Ceníkem, zrušené subscription se na stránce nepovažují za aktivní a LIVE webhook před DB syncem ověřuje aktuální canonical subscription stav ve Stripe. Veřejně zobrazovaná verze na dashboardu zůstává 0.9.20.
...
6. **live billing je veřejný a lifecycle e-maily mají produkční E2E acceptance COMPLETE / PASS**; 0.9.22 opravuje první produkční regresi správy předplatného a drží least-privilege Stripe key. Před mergem je nutné doplnit na LIVE restricted key `Subscriptions → Write`, `Subscription Schedules → Write` a `Invoices → Read`; poté ověřit stránku a teprve následně první skutečnou změnu tarifu. Změna země/měny zůstává řízená. Team / School / Campus zatím nezapínat;
