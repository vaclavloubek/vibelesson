...
- Resend automatizace **Syllonaut · Organization owner activated** (`01a0cfba-5e81-7092-be05-0d4cd3948a6f`; opt-in → `ui_locale` → `event.plan_code == campus` → 4 šablony CS/EN × School/Campus) a **Syllonaut · Organization member joined** (`01a0cfba-75b8-760c-889b-c8a2337ac501`; opt-in → `ui_locale` → 2 šablony) jsou připravené a **disabled**; zapnou se až po nasazení této verze;
...
- produkčně jsou aktivní (stav 2026-09-23) tyto automations: **Syllonaut · Onboarding**, **Syllonaut · First live → Paid**, **Syllonaut · Free quota near limit**, **Syllonaut · Free quota reached**, **Syllonaut · Paid onboarding**, **Syllonaut · Win-back after cancellation** a **Syllonaut · Renewal tips before billing period ends**;
...
- **Organization owner activated** a **Organization member joined** (0.9.111) jsou připravené, ale zatím **disabled** — nejsou součástí 7 aktivních automations (detail v sekci 0.9.111);
...
- produkční `RESEND_API_KEY` musí mít **Full access** (klíč „Syllonaut app“ od 2026-09-24); klíč jen pro odesílání vrací u Contacts a Events API `401 restricted_api_key` a lifecycle tiše nefunguje;
...
- dočasné review/test automations (`TEST · …`) jsou po testu **disabled**; všech 7 produkčních automations uvedených výše je **enabled**.
