...
requireText(lessonContentWriter, "process.env.VERCEL_ENV === 'production'", 'The Neon lesson-content canary must be isolated from unapproved production use.');
requireText(lessonContentWriter, 'where id = ${lessonId}', 'The Neon lesson-content write must remain lesson-scoped and parameterized.');
requireText(lessonContentWriter, 'and owner_id = ${userId}', 'The Neon lesson-content write must remain owner-scoped and parameterized.');
requireText(lessonContentWriter, 'lesson = ${JSON.stringify(lesson)}::jsonb', 'The Neon lesson document must remain parameterized.');
requireText(lessonContentRoute, 'readLessonContentForWrite(supabase, userId, id)', 'The lesson edit route must read from the selected write backend.');
