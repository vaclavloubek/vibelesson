...
requireText(guide, 'PHONE_PRESENTER_MEDIA', 'shared phone Presenter media query');
requireText(guide, "state.chapter !== 'live' || state.step >= 2", 'phone guide must skip Presenter setup steps');
requireText(guide, "state.chapter === 'live' && phonePresenterHidden && previousStep < 2", 'phone guide back navigation must not return to Presenter steps');
requireText(guide, 'steps.length - 2', 'phone guide progress must exclude Presenter-only steps');
...
requireText(css, '.phone-hide-presenter { display: none !important; }', 'phone Presenter controls must be removed from layout');
requireText(css, '@media (max-width: 680px) and (hover: none), (max-height: 500px) and (hover: none)', 'phone-only Presenter breakpoint must preserve tablet layouts');
...
requireText(helpButton, 'className?: string;', 'contextual help must accept device-specific visibility classes');
