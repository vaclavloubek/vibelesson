...
function anonymousDelta(sourceRows, targetRows) {
  ...
  for (const row of sourceRows) sourceOwners.set(row.teacher_id, (sourceOwners.get(row.teacher_id) ?? 0) + 1);
  for (const row of targetRows) targetOwners.set(row.teacher_id, (targetOwners.get(row.teacher_id) ?? 0) + 1);
...
const source = new Client({
  ...
  application_name: 'syllonaut-neon-session-history-parity',
});
...
`;
const ownerColumns = allColumns.replace('teacher_id::text as teacher_id,', '');
...
try {
  ...
  if (fingerprint(canonicalRows(sourceAll.rows)) !== fingerprint(canonicalRows(targetAll))) {
    ...
    console.error(`Ended-session row counts: Supabase ${sourceAll.rows.length}, Neon ${targetAll.length}.`);
    ...
    throw new Error('Supabase and Neon ended-session tables have different fingerprints.');
  }

  const ownerId = targetAll[0]?.teacher_id;
  if (typeof ownerId !== 'string') throw new Error('No eligible ended-session owner exists for parity.');

  const sourceFiltered = await source.query(`
    select ${ownerColumns}
    from public.sessions
    where teacher_id = $1 and status = 'ended' and ended_at is not null
  ...
  if (fingerprint(canonicalRows(sourceFiltered.rows)) !== fingerprint(canonicalRows(targetFiltered))) {
    throw new Error('Supabase and Neon returned different ended-session rows for the same owner.');
  }

  await source.query('commit');
  console.log(`Compared ${targetAll.length} ended-session rows across both databases.`);
  console.log(`Compared ${targetFiltered.length} owner-scoped ended-session rows without logging their owner or contents.`);
  console.log('PASS: Supabase and Neon returned the same ended-session table and owner-scoped result.');
  console.log('Read-only parity check completed; no owner ID, join code, lesson snapshot, or secret was logged.');
} catch (error) {
