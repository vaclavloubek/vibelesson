...
const source = new Client({
  ...
  application_name: 'syllonaut-neon-lesson-worksheet-parity',
});
...
try {
  ...
  if (fingerprint(canonicalRows(sourceAll.rows)) !== fingerprint(canonicalRows(targetAll))) {
    ...
    console.error(`Lesson-worksheet row counts: Supabase ${sourceAll.rows.length}, Neon ${targetAll.length}.`);
    console.error(`Anonymous delta: Supabase-only ${delta.sourceOnly}, Neon-only ${delta.targetOnly}, changed ${delta.changed}.`);
    throw new Error('Supabase and Neon lesson-worksheet tables have different fingerprints.');
  }

  const candidate = targetAll[0];
  if (typeof candidate?.owner_id !== 'string' || typeof candidate?.id !== 'string') {
    throw new Error('No eligible lesson worksheet exists for parity.');
  }

  const sourceLesson = await source.query(`
    select
      id::text as id,
      lesson
    from public.lessons
  ...
  const targetLesson = await target`
    select
      id::text as id,
      lesson
    from public.lessons
  ...
  if (fingerprint(sourceLesson.rows) !== fingerprint(targetLesson)) {
    throw new Error('Supabase and Neon returned different worksheet content for the same owner-scoped lesson.');
  }

  await source.query('commit');
  console.log(`Compared ${targetAll.length} lesson-worksheet rows across both databases.`);
  console.log('Compared one owner-scoped worksheet lesson without logging its lesson ID, owner, or content.');
  console.log('PASS: Supabase and Neon returned the same lesson-worksheet table and owner-scoped result.');
  console.log('Read-only parity check completed; no lesson ID, owner ID, content, or secret was logged.');
} catch (error) {
