...
requirePattern(publicPage, /<ImportSharedLessonButton token=\{token\} importRequested=\{importRequested\}/, 'the shared page must pass import intent to the client auth-aware import button.');
...
requirePattern(sharedAuthControls, /const handleAuthChange = useCallback\(\(\) => \{\}, \[\]\)/, 'shared import resumption must not depend on an auth callback reload.');
