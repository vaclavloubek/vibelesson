...
requirePattern(sharedAuthControls, /headers:[\s\S]*Authorization:[\s\S]*Bearer[\s\S]*accessToken/, 'shared lesson sign-in must import with the fresh JWT instead of waiting for cookie propagation.');
...
requirePattern(importButton, /headers: accessToken[\s\S]*Authorization:[\s\S]*Bearer[\s\S]*accessToken/, 'manual shared imports must send the browser JWT to the import API.');
...
requirePattern(version, /APP_VERSION = '0\.9\.140'/, 'the current public release must publish dashboard version 0.9.140.');
