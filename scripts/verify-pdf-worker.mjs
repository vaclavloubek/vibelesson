...
requireText(materialsClient, "const PDF_WORKER_URL = '/pdf.worker.mjs';", 'browser PDF extraction must use the same-origin worker asset.');
...
requireText(copyScript, 'node_modules/pdf-parse/dist/pdf-parse/web/pdf.worker.mjs', 'worker must come from the installed pdf-parse package.');
requireText(copyScript, "resolve(targetDir, 'pdf.worker.mjs')", 'worker must be copied to the public same-origin path.');
requireText(gitignore, 'public/pdf.worker.mjs', 'generated worker asset must not be committed.');
...
await access(new URL('../node_modules/pdf-parse/dist/pdf-parse/web/pdf.worker.mjs', import.meta.url));
