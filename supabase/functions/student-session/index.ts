...
async function chooseTeam(b: Record<string, unknown>) {
  ...
  const [{ data: s, error: se }, { data: team, error: te }] = await Promise.all([db.from("sessions").select("status,realtime_key").eq("id", sessionId).maybeSingle(), db.from("teams").select("id,name").eq("id", teamId).eq("session_id", sessionId).maybeSingle()]); if (se || te) return reply({ error: "Tým se nepodařilo načíst." }, 500); if (!s) return reply({ error: "Hodina neexistuje." }, 404); if (!team) return reply({ error: "Tým v této hodině neexistuje." }, 404); if (s.status === "ended") return reply({ error: "Tato hodina už skončila." }, 410); if (s.status === "live" && p.team_id && p.team_id !== teamId) return reply({ error: "Po zahájení hodiny už tým změnit nejde." }, 409); const { error } = await db.from("participants").update({ team_id: teamId }).eq("id", p.id).eq("session_id", sessionId); if (error) return reply({ error: "Tým se nepodařilo vybrat." }, 500); await invalidate(s.realtime_key as string); return reply({ ok: true, team: { id: team.id, name: team.name } });
}
...
async function respond(b: Record<string, unknown>) {
  ...
  const { data: saved, error: saveError } = await db.from("responses").upsert({ session_id: sessionId, participant_id: p.id, block_id: blockId, answer: n.answer, updated_at: new Date().toISOString() }, { onConflict: "session_id,participant_id,block_id" }).select("answer,updated_at").single(); if (saveError || !saved) return reply({ error: "Odpověď se nepodařilo uložit." }, 500); await invalidate(s.realtime_key as string); return reply({ ok: true, blockId, answer: saved.answer, updatedAt: saved.updated_at });
}
