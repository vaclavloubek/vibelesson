alter table public.billing_plans
  add column if not exists multilingual_lessons_enabled boolean not null default false;

alter table public.profiles
  add column if not exists multilingual_lessons_enabled boolean not null default false;

alter table public.manual_entitlement_overrides
  add column if not exists multilingual_lessons_enabled boolean;

update public.billing_plans
set multilingual_lessons_enabled = (code in ('teacher', 'teacher_pro', 'admin')),
    updated_at = now()
where multilingual_lessons_enabled is distinct from (code in ('teacher', 'teacher_pro', 'admin'));

update public.profiles p
set multilingual_lessons_enabled = bp.multilingual_lessons_enabled,
    updated_at = now()
from public.billing_plans bp
where bp.code = p.active_plan_code
  and p.multilingual_lessons_enabled is distinct from bp.multilingual_lessons_enabled;

comment on column public.billing_plans.multilingual_lessons_enabled is
  'Plan entitlement for generating lessons in a language different from the active UI locale, including automatic prompt-language detection.';

comment on column public.profiles.multilingual_lessons_enabled is
  'Server-authoritative effective entitlement for multilingual lesson generation.';

comment on column public.manual_entitlement_overrides.multilingual_lessons_enabled is
  'Optional manual override for multilingual lesson generation entitlement.';
...
begin
  ...
  v_multilingual := v_plan.multilingual_lessons_enabled;
  ...
  if found then
    ...
    if v_override.multilingual_lessons_enabled is not null then
      v_multilingual := v_override.multilingual_lessons_enabled;
    end if;
  end if;
