...
set monthly_ai_grading_count_limit = case code
      when 'teacher_pro' then 60
      when 'school' then 300
      when 'campus' then 750
      else monthly_ai_grading_count_limit
    end,
    monthly_ai_grading_budget_usd = case code
      when 'teacher_pro' then 12.00
      when 'school' then 60.00
      when 'campus' then 150.00
...
begin
  ...
  if v_user_id is null or not v_ai_enabled then return false; end if;
  if v_role = 'admin' then return true; end if;
  if private.effective_ai_billing_paused(v_user_id) then return false; end if;
  ...
  if v_org_id is not null then
    ...
    select coalesce(p.active_plan_code, 'free') into v_plan_code from public.profiles p where p.id = v_user_id;
  ...
  select bp.monthly_ai_grading_budget_usd, bp.monthly_ai_grading_count_limit
  into v_budget, v_count_limit
  from public.billing_plans bp
  where bp.code = v_plan_code and bp.ai_grading_enabled;

  if v_budget is null or v_count_limit is null then return false; end if;
  ...
  where r.status = 'reserved'
    ...
    and (
      ...
      or (v_org_id is null and r.organization_id is null and r.user_id = v_user_id)
    );

  if exists (
    select 1 from private.ai_grading_budget_requests r
    where r.evaluation_id = p_evaluation_id and r.status = 'reserved'
  ) then return true; end if;

  select count(*)::integer,
         coalesce(sum(case when r.status = 'reserved' then r.reserved_cost_usd else coalesce(r.actual_cost_usd, 0) end), 0)::numeric(14,6)
  ...
  if v_used_count >= v_count_limit or v_used_cost + v_reservation > v_budget then return false; end if;

  insert into private.ai_grading_budget_requests (
    evaluation_id, user_id, organization_id, plan_code, status, reserved_cost_usd
  ) values (
    p_evaluation_id, v_user_id, v_org_id, v_plan_code, 'reserved', v_reservation
  );
...
begin
  ...
  if v_org_id is not null then
    ...
    select count(*)::integer into v_lesson_used from public.generation_requests g
    where g.organization_id = v_org_id and g.action = 'generate_lesson'
      and g.status in ('pending', 'succeeded') and g.created_at >= v_window_start and g.created_at < v_window_end;

    select count(*)::integer into v_revision_used from public.generation_requests g
    where g.organization_id = v_org_id and g.action in ('revise_lesson', 'revise_block')
      and g.status in ('pending', 'succeeded') and g.created_at >= v_window_start and g.created_at < v_window_end;
  else
    ...
    select count(*)::integer into v_lesson_used from public.generation_requests g
    where g.user_id = v_user_id and g.organization_id is null and g.action = 'generate_lesson'
      and g.status in ('pending', 'succeeded') and g.created_at >= v_window_start and g.created_at < v_window_end;

    select count(*)::integer into v_revision_used from public.generation_requests g
    where g.user_id = v_user_id and g.organization_id is null and g.action in ('revise_lesson', 'revise_block')
      and g.status in ('pending', 'succeeded') and g.created_at >= v_window_start and g.created_at < v_window_end;
  end if;

  select bp.ai_grading_enabled, bp.monthly_ai_grading_count_limit
  into v_grading_enabled, v_grading_limit
  from public.billing_plans bp where bp.code = v_plan_code;
  ...
  return query select
    ...
    v_grading_used,
    v_grading_limit,
    case when not v_grading_enabled or v_grading_unlimited or v_grading_limit is null then null
         else greatest(v_grading_limit - v_grading_used, 0) end,
    v_grading_unlimited,
    v_grading_enabled,
    v_window_start,
    v_window_end,
...
grant execute on function private.reserve_ai_grading_budget(uuid) to postgres;
