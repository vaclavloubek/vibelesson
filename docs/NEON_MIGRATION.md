...
17. [Aktivováno v Preview] `NEON_LIVE_SESSION_DATA=true` přesměruje všechny nové studentské a týmové Vercel moduly včetně veřejného scoreboardu na parametrizované server-only Neon SQL/RPC. Výchozí stav je `false`, Supabase fallback zůstává zachovaný a produkční aktivace bez `NEON_CUTOVER_APPROVED=true` selže zavřeně. Migrace `0007_live_session_grading_outbox.sql` nahradila `net.http_post` privátním outboxem, concurrency-safe `FOR UPDATE SKIP LOCKED` claimem a Vercel cron workerem. Jednorázový capability token se vrací pouze workeru v rámci serverového DB spojení; databáze ukládá výhradně jeho SHA-256 hash. Klientské Data API granty se neotevírají. Transakční instalační deployment `3cr47vTJFg3bfW3tbxLNSxMnM8uN` prošel SQL postconditions a následný čistý deployment `55gjj1V3y52DyKyCTnuZcYoNzw6N` skončil `Ready`; oba přepínače jsou omezené pouze na Preview větev `codex/neon-staging-import-20260921-v2`.
18. [Částečný browser smoke test, 2026-09-22] Na aktuálním Preview deploymentu `ChoVoAUt8byzFag2ssGwdVfwZyBg` se načetla stránka `/join`, formulář přijal syntakticky platný neexistující kód `Q7X9K2M` a po odeslání zobrazil očekávané „Hodina s tímto kódem neexistuje.“ Vercel runtime log potvrzuje `POST /api/student/join` → `404` ve větvi `codex/neon-staging-import-20260921-v2`. V Neon větvi `preview/codex/neon-staging-import-20260921-v2` je 23 ukončených relací, 151 účastníků, 265 odpovědí, 77 historických hodnocení, ale 0 aktivních relací a 0 outbox úloh. SQL kontrola potvrdila, že `private.claim_next_grading_outbox_job(text,integer)` nemohou spustit role `anon` ani `authenticated`, zatímco serverová vlastnická role ano. Tento test **neověřuje** úspěšný join, studentskou odpověď, týmový tok, teacher start/next/end ani skutečné AI hodnocení; ty zůstávají před cutoverem povinné.

Hlavní Preview přihlášení aktuálně používá produkční Supabase Auth a hardcoded Turnstile widget; uživatel v tomto toku neprojde. Izolovaná stránka `/auth/neon-staging` má funkční Neon Auth login bez Turnstile, ale její cookie zatím hlavní aplikační stránky nepřijímají. Nelze z ní proto odvozovat úspěšný učitelský end-to-end test. Pouhé vypnutí Turnstile nebo povolení Preview hostu u produkčního Supabase widgetu není řešením migrace: autentizace by stále mířila na Supabase, zatímco vybrané datové zápisy už míří na Neon staging. Před plným testem je nutné sjednotit Auth a datový backend v izolovaném prostředí.

### Krok 1: sjednocení aplikačního backendu (rozpracováno)

Lokálně je připravený globální režim `DATABASE_BACKEND=neon` se shodným veřejným přepínačem `NEXT_PUBLIC_DATABASE_BACKEND=neon` a povinným `NEON_CUTOVER_APPROVED=true`. Výchozí režim zůstává Supabase a v žádném prostředí nebyl globální Neon režim nasazen. Serverový request-scoped klient bere JWT z Neon Auth relace a používá Neon Data API; identitu ověřuje i proti `app_identity.users`. Hlavní přihlášení existujících uživatelů používá first-party Neon Auth server action, která nepřebírá Supabase session. Předčasný přístup do dosud nepřevedených privilegovaných cest končí chybou místo zápisu do staré databáze.

V globálním režimu Neon se již ověřené úzké Neon služby pro lekce, složky, sdílení a live-session data zapínají automaticky; jejich jednotlivé Preview canary přepínače zůstávají zachovány pro výchozí Supabase režim. Doplněné serverové SQL větve pokrývají přijetí VOP, registraci a správu důvěryhodných zařízení, stav pozastavení AI kvůli platbě, aktuální školní členství a kontrolu původu školní lekce. Import sdílené lekce v Neon režimu vyžaduje first-party cookie a odmítá Supabase bearer token, aby nedošlo ke smíšení identit.

Toto **není hotový aplikační port**: registrace a obnova hesla v hlavním Auth UI zatím nejsou bezpečně převedené včetně ochrany proti automatizaci a právních záznamů. Přibližně 62 serverových modulů stále obsahuje volání `createAdminClient`; z nich 38 jsou API route handlery. Některé už mají samostatnou Neon větev, zbývající však při globálním přepnutí úmyslně selžou. Mezi zbývajícími kritickými oblastmi jsou billing/Stripe, organizace, marketingový lifecycle, admin a část generování. TypeScript i úplná stávající regresní sada procházejí. Lokální `next build` se ale při optimalizaci bez chybového výpisu zastavil v Turbopack i webpack variantě a byl ukončen; nelze jej považovat za zelený. Nový globální režim navíc ještě neprošel browser → API → Neon testem. Produkční přepnutí je proto dál zakázáno.

V navazující práci byly do serverového Neon SQL přesměrovány také read-only stav individuálního předplatného, vytvoření Stripe Checkout s atomickým uložením smluvního snapshotu, otevření Stripe Customer Portalu a změna individuálního předplatného. Stripe webhook má nově úzký allowlist devíti privilegovaných synchronizačních funkcí s parametrizovanými argumenty; načtení organizační fakturační trasy a idempotentní evidence platební události používají ve větvi Neon přímé SQL. Navazující e-mailový lifecycle pro individuální předplatné má Neon větev pro idempotentní frontu, ověření smluvního snapshotu, příjemce z identity bridge a evidenci doručení. Marketingový lifecycle nyní čte profil, identitu a počty prvních lekcí/relací také z Neonu. Všechny cesty zachovávají Supabase jako výchozí backend a pro Neon vyžadují schválený globální cutover. `npm run check` po změnách prošel. **Neznamená to dokončení platebního portu**: školní fakturace, výběry a další právní operace v jiných modulech stále používají privilegovaného Supabase klienta. Bez jejich převodu se produkce nesmí přepnout. Read-only přímá kontrola schématu Neonu nebyla možná, protože lokálně dostupná `NEON_DATABASE_URL_UNPOOLED` nemá hodnotu; nebyl proveden žádný databázový zápis.

Další lokální port (2026-09-23) nahradil část privilegovaných `.rpc()` volání společnou serverovou Neon vrstvou s pevným allowlistem funkcí, validací názvů argumentů a parametrizovanými hodnotami. Zahrnuje právní notifikace a odstoupení, školní pozvánky a vlastníka, bankovní párování, kontaktní formulář, cron školní fakturace, založení live relace, regrade a rezervaci/dokončení AI generování. Funkce `RETURNS TABLE` pro rezervaci AI kvóty zachovávají pole výsledků jako PostgREST. Serverové SQL nyní obsluhuje také vložení vygenerované lekce, důkazy a zpracování individuálního odstoupení, refundaci při změně služby, vystavení/načtení školní bankovní faktury a část cron obnovy školních licencí. Původní Supabase backend zůstává výchozí. `npm run check` a `npm run check:neon-migration` procházejí; statický audit našel všechny funkce z allowlistu v migračních SQL a dvě funkce s tabulkovým výsledkem jsou explicitně ošetřené. **Stále to není schválení cutoveru**: další školní endpointy, Auth registrace/reset, některé administrativní cesty a runtime integrace musí být portovány a ověřeny proti skutečnému Neonu. Lokální přímé DB URL a zdrojový DB URL jsou v dostupném `.env.local` prázdné; nový port proto nemá databázový ani browser E2E důkaz. Produkční prostředí nebylo změněno.

Navíc byly lokálně přesměrovány tvorba školní objednávky, opakování kartové platby, čtení a PDF školní bankovní faktury a ukládání identifikátorů Stripe zákazníka/Checkout do příslušné databáze. `npm run check` prošel i po této změně. Školní účet a jeho ostatní administrativní endpointy však stále nejsou kompletně portované. Lokální `npm run build` v tomto kroku skončil chybou při stahování fontů Geist/Geist Mono z Google Fonts kvůli síťovému připojení v prostředí; nedošlo až k úplnému ověření buildu a tento výsledek není zelený build ani důkaz chyby portu. Před cutoverem je nutný úspěšný Preview build a E2E kontrola s Neon backendem.

Dne 2026-09-23 prošel celý lokální `npm run check` po serverovém portu. Ověřený strom změn byl bezpečně sloučen s novějším stavem vzdálené Preview větve a commit `ab3ed17e8a49f4c7b054e3330611a8c909bedec5` úspěšně sestavil Vercel Preview deployment `E6G4UFNcvuQ4G56nV2KqFUVfNvKU` (`Ready`). Na jeho jedinečné URL se načetla neveřejná stránka `/auth/neon-staging`; nebylo provedeno přihlášení, registrace ani databázový zápis. Hlavní aplikační Neon Auth tok zůstává blokovaný: registrace a obnova hesla ještě nejsou bezpečně zapojené a vyhledání `TURNSTILE` v proměnných projektu `edupilot2` ve Vercelu nevrátilo žádný výsledek. Úspěšný build a načtení stagingové stránky proto nejsou end-to-end důkazem ani souhlasem s produkčním cutoverem. Produkční prostředí zůstalo beze změny.

Navazující stagingový checkpoint (2026-09-23): hlavní formulář ve větvi `codex/neon-staging-import-20260921-v2` nyní nabízí Neon registraci a obnovu hesla, vyžaduje serverové ověření Turnstile pro login, registraci i reset a zapisuje profil i právní/marketingový audit nové registrace. Existující widget Cloudflare dovoluje pouze produkční doménu a přesnou stabilní stagingovou doménu; jeho serverový klíč a přepínače `DATABASE_BACKEND=neon`, `NEXT_PUBLIC_DATABASE_BACKEND=neon`, `NEON_CUTOVER_APPROVED=true` jsou ve Vercelu omezené na tuto Preview větev. Produkční konfigurace zůstala beze změny. Preview build s těmito změnami byl `Ready`. Přihlášená Neon session v Safari prošla hlavní aplikací až na seznam lekcí; původní blokaci způsobila chybějící dávková funkce pro kontrolu VOP, takže Neon větev nyní bezpečně čte importovaný audit přímo. Po opravě načítání skriptu Turnstile se na stabilní Preview doméně ověření přihlášení samo dokončilo a pokus s neexistujícím syntetickým účtem byl správně odmítnut. V soukromém Safari interaktivní Turnstile pro obnovu hesla uspěl, hlavní aplikace potvrdila žádost a Gmail následně ukázal nový e-mail `Reset Your Password - neon-red-ladder` pro stagingový účet. Samotné otevření odkazu a nastavení nového hesla není ověřené, protože finální změnu hesla musí provést vlastník účtu. Registrace také nemá plný E2E důkaz. Spravované Neon Auth má vlastní veřejný endpoint a zákaz přímé registrace v aplikačním proxy neprokazuje ochranu proti automatizaci na tomto endpointu. Stagingové přesměrování resetu používá stabilní `VERCEL_BRANCH_URL`, která je v Neon Auth důvěryhodných doménách.

Pro soukromý Safari test byl na stabilní Preview větvi dočasně vytvořen Vercel Shareable Link. Po potvrzeném doručení e-mailu byl s výslovným souhlasem vlastníka zrušen; Vercel znovu ukázal `Only people with access`. Žádné produkční přístupové nastavení se neměnilo.

Read-only porovnání 14 hlavních tabulek ukázalo 10 shodných otisků a drift ve čtyřech: Supabase/Neon `profiles` 4/3, `lessons` 33/31, `sessions` 24/23 a `lesson_shares` 9/8. V Neonu jsou 3 Auth účty, ve zdroji 4. **Finální dosynchronizace nebyla provedena**: produkční Supabase dál přijímá zápisy a současná Neon staging větev už obsahuje resetované Auth heslo; opakování původního importu do neprázdné větve by nebylo bezpečné. Před finální kopií je nutné schválené write-freeze okno a záloha/snapshot obou stran, pak přenos úplné delty se zachováním Neon-only credential stavu a opětovná kontrola otisků. Stále platí stop podmínky pro produkční cutover v části níže.

Serverová konfigurace Neonu umí číst i Vercel integrací vytvořený název `NEON_DATABASE_DATABASE_URL` a před jakýmkoli SQL spojením odmítá jiný než `postgres(ql)` protokol a host mimo `.neon.tech`. Tím se předchází náhodnému zápisu do Supabase přes obecný `DATABASE_URL`. Přímé Vercel CLI spojení pro ověření názvů aktuálních Preview proměnných skončilo síťovou chybou `fetch failed`; propojený projektový konektor měl nekompatibilní validaci vstupu. Rozsah aktuálních Preview proměnných tedy nebyl znovu ověřen a nic ve Vercelu se neměnilo.
...
- [Aktivováno v Preview] `student-session` a `team-edit` mají server-only Neon SQL/RPC backend za `NEON_LIVE_SESSION_DATA=true`, Supabase fallbackem a produkční pojistkou;
...
- [Aktivováno v Preview] `net.http_post` grading dispatch nahrazuje privátní DB outbox a autorizovaný Vercel worker; plaintext capability se neukládá;
...
- Opraveno 2026-09-23: `reconcile_live_control_snapshot` na Neonu padal (`operator does not exist: text = jsonb`). PostgreSQL 18 váže nekvalifikované `value` na vnitřní jsonb sloupec; migrace `0009_pg18_live_reconciliation.sql` alias kvalifikuje a triggery živých zápisů místo role `postgres` rozpoznávají vlastníka rekonciliační funkce. Aplikováno ostře po zkoušce s rollbackem (diagnostika na PG 18.6 potvrdila `42883` u původního dotazu). Trigger `private.reject_individual_contract_evidence_mutation` povoluje DELETE jen roli `postgres`, na Neonu tedy nikomu. Záměrně se nemění: snapshoty nemají kaskádovou vazbu na uživatele a aplikace je nemaže, takže smazání účtu neblokují, a aplikace na Neonu běží pod vlastnickou rolí, takže výjimka pro ni by append-only ochranu oslabila. Případný retenční výmaz provede správce ručně v jedné transakci: `begin; alter table private.individual_contract_snapshots disable trigger individual_contract_snapshots_append_only; delete … ; alter table private.individual_contract_snapshots enable trigger individual_contract_snapshots_append_only; commit;` (tabulky odstoupení a refundů na snapshot odkazují s `on delete restrict`).
- Opraveno 2026-09-23: `public.sync_stripe_subscription_event` (Stripe webhook) kontroloval `auth.role()`, které Neon (`pg_session_jwt`) nemá; každá synchronizace předplatného by selhala s `42883` (potvrzeno diagnostikou v produkci). Migrace `0010_neon_stripe_sync_caller_check.sql` kontrolu nahradila odmítnutím přihlašovacích rolí Data API (`session_user`); test v migraci ověřil, že volání ze serveru projde kontrolou volajícího a odmítne neplatné ID události (`22023`). V databázi už není žádný jiný odkaz na `auth.role()`.
- 2026-09-23: v integraci Vercel↔Neon je pro Preview vypnuté vytváření databázové větve i povinnost aktivního resource (`deployments.actions = []`, `required = false`, ověřeno přes Vercel API). Běžné PR Preview Neon nepoužívají; tím zmizely chyby „Resource provisioning failed“ a hromadění větví na Free tarifu. Preview buildy staré migrační git větve `codex/neon-staging-import-20260921-v2` od té doby dostávají připojení k Neon větvi `main`, nikoli k produkční; její build hooky (`NEON_*_MODE`) proto znovu nepoužívat.
- 2026-09-26 (0.9.162): **plná refundace individuálního předplatného ho okamžitě ukončí.** Stripe webhook po úspěšném `sync_stripe_refund_state` (jen individuální platby, jen LIVE, jen plná refundace; ne školy ani balíčky AI hodnocení) načte ze Stripe refundace dané charge, předplatné a fakturu refundované platby (`invoice_payments`). Předplatné ve stavu `trialing`/`active`/`past_due` zruší stejně jako online odstoupení (`DELETE`, `invoice_now=false`, `prorate=false`, idempotency key `syllonaut-full-refund-cancel-<sub>`), jen když refundovaná platba patří k jeho `latest_invoice`. Nic neruší: důvod `duplicate`, částečná, neúspěšná nebo zrušená refundace, refundace starší faktury (jen varování v logu) a refundace s metadaty `syllonaut_withdrawal_id` / `syllonaut_service_change_termination_id` (ty ruší vlastní cesta). Opakované `charge.refunded` / `refund.created` / `refund.updated` najdou předplatné zrušené a nic neudělají. Selhání Stripe vrátí 500 a Stripe událost zopakuje (zápis refundace je idempotentní). E-mail „Předplatné bylo ukončeno“ a `syllonaut.subscription.ended` pošle beze změny existující cesta `customer.subscription.deleted`. Logika: `lib/stripe-full-refund-cancellation.ts`, test `scripts/verify-full-refund-subscription-cancellation.mjs` (spouští skutečnou route). Migrace `0022_refund_pause_requires_live_individual_subscription.sql`: `private.individual_ai_billing_pause_reason` vrací `'refund'` jen při zároveň živém individuálním předplatném, takže uživatel na Free už nemá AI pozastavené kvůli staré refundaci. Starší refundace se zpětně nezpracují; předplatná, která po nich běží dál, se ruší ručně.
- 2026-09-26 (0.9.163): migrace `0023_free_single_use_notice_acknowledgements.sql` přidává append-only tabulku `private.free_single_use_notice_acknowledgements` (jen pro server, RLS, bez práv pro API role, bez FK, trigger odmítá UPDATE/DELETE). Zapisuje se do ní zavření upozornění Free „každou lekci živě použít jednou“ (tlačítko „OK, rozumím“ nebo křížek) s přesným textem upozornění a časem serveru; detail lekce podle ní upozornění skryje. Data API se nemění, obnova schema cache není potřeba. Aplikováno v produkci 2026-09-26.
- **Provozní pravidlo pro refundace ve Stripe dashboardu:** vždy vybrat důvod. **Duplicate** jen pro dvojitou platbu v rámci *téhož* předplatného (období zůstává zaplacené druhou platbou): předplatné běží dál a od 0.9.164 webhook pauzu AI hned uvolní (`release_stripe_duplicate_payment_refund`), pokud má předplatné jinou nevrácenou platbu v téže měně, aspoň ve výši vrácené částky, zaplacenou do 7 dnů od vrácené. Když dvojitý checkout vytvořil **druhé předplatné**, vrátit jeho platbu s důvodem **Requested by customer**: druhé předplatné se okamžitě zruší a pauza AI se kvůli němu neuplatní, protože od migrace `0024` platí jen při živém *refundovaném* předplatném. Ve všech ostatních případech **Requested by customer**: plná refundace platby za aktuální období předplatné okamžitě ukončí a účet přejde na Free (VOP 1.13, čl. 6). Bez vybraného důvodu se plná refundace aktuální faktury bere jako ukončení.
- Otevřené: za provozu neověřené školní administrace, Stripe webhook a registrace; ostatní účty resetují heslo. (Původně uváděné limity tarifu Free už neplatí: organizace je na tarifu **Launch**, ověřeno 2026-09-25.)
...
Vlastník projektu může být prvním a jediným člověkem, který po přepnutí vyzkouší živou lekci v Production. To však není náhrada předprodukčních technických kontrol: před přepnutím musí být dokončen port hlavního Auth a zbývajících Supabase datových cest, finální synchronizace dat, kontrola platebních a právních zápisů i ověřený návratový postup. Samotná úspěšná lekce neprokazuje, že ostatní produkční toky používají Neon. Test vlastníka proto proběhne až během řízeného údržbového okna, než se znovu otevřou běžné zápisy. Současný stav tyto podmínky nesplňuje a tento odstavec není souhlasem k okamžitému cutoveru.
