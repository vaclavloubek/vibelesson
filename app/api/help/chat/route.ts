...
export async function POST(req: Request) {
  ...
  let finished = false;
  async function finish(status: 'succeeded' | 'failed', costUsd: number | null, topic: HelpTopic | null) {
    if (finished) return;
    finished = true;
  ...
  const body = createHelpResponseBody({
    ...
    logTiming: (outcome, timing) => console.info('help assistant stream timing', { outcome, ...timing }),
  });
