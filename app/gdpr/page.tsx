...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: english
      ? 'How Syllonaut processes personal data, Terms and paid-contract evidence, cookies, account data, live lesson data, contact enquiries and marketing consent.'
      : 'Jak Syllonaut zpracovává osobní údaje, záznamy o přijetí podmínek a placených smlouvách, cookies, údaje z účtů, živých lekcí a marketingových souhlasů.',
    alternates: {
    ...
    <main className={landing.page}>
      ...
      <article className={styles.page}>
        <div className={styles.hero}>
          ...
          <div className={styles.meta}>{ui('Verze 1.3 · účinná od 21. 9. 2026', 'Version 1.3 · effective from 21 September 2026')}</div>
        </div>
        ...
        <section>
          ...
          <ul>
            ...
            <li><strong>{ui('Potvrzení placené smlouvy:', 'Paid-contract confirmation:')}</strong> {ui('u individuálního placeného tarifu ukládáme technický identifikátor účtu, tarif, fakturační období, měnu a cenu, verzi / acceptance key přijatých VOP, serverový čas přijetí, neměnnou kopii smluvních informací a přijatých VOP, vzorový formulář pro odstoupení a kontrolní SHA-256 hash. Do tohoto smluvního snapshotu nekopírujeme e-mail zákazníka.', 'for an individual paid plan we retain the technical account identifier, plan, billing period, currency and price, accepted Terms version / acceptance key, server acceptance time, an immutable copy of the contract information and accepted Terms, the model withdrawal form and a verification SHA-256 hash. The customer email address is not duplicated in this contract snapshot.')}</li>
        ...
        <section>
          ...
          <div className={styles.tableWrap}><table><thead><tr><th>{ui('Účel', 'Purpose')}</th><th>{ui('Právní základ', 'Legal basis')}</th></tr></thead><tbody>
            ...
            <tr><td>{ui('Evidence přijetí obchodních podmínek, obsahu placené smlouvy a ochrana právních nároků', 'Recording acceptance of the Terms, the content of a paid contract and protecting legal claims')}</td><td>{ui('Plnění smlouvy a oprávněný zájem na doložení vzniku a přesného obsahu smluvního vztahu a na určení, výkonu nebo obhajobě právních nároků (čl. 6 odst. 1 písm. b a f GDPR)', 'Performance of the contract and legitimate interest in evidencing the formation and exact content of the contractual relationship and in establishing, exercising or defending legal claims (Art. 6(1)(b) and (f) GDPR)')}</td></tr>
        ...
        <section>
          ...
          <ul>
            ...
            <li>{ui('Neměnný snapshot individuální placené smlouvy (technické ID účtu, parametry objednávky, přijaté VOP, formulář pro odstoupení, serverový čas a kontrolní hash) můžeme obdobně uchovat i po zrušení účtu pouze po dobu, kdy je to přiměřeně nutné k doložení uzavřené smlouvy, splnění souvisejících právních povinností nebo k určení, výkonu či obhajobě právních nároků; poté jej smažeme nebo anonymizujeme v rozsahu, v jakém už identifikace není potřebná.', 'An immutable snapshot of an individual paid contract (technical account ID, order parameters, accepted Terms, withdrawal form, server timestamp and verification hash) may likewise be retained after account deletion only for as long as reasonably necessary to evidence the concluded contract, meet related legal obligations or establish, exercise or defend legal claims; it is then deleted or anonymised to the extent identification is no longer needed.')}</li>
