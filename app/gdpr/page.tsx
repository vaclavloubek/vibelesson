    ...
    <main className={landing.page}>
      ...
      <article className={styles.page}>
        <div className={styles.hero}>
          ...
          <div className={styles.meta}>{ui('Verze 1.4 · účinná od 21. 9. 2026', 'Version 1.4 · effective from 21 September 2026')}</div>
        </div>
        ...
        <section>
          ...
          <p>{ui('U údajů, které Syllonaut zpracovává pro vlastní účely (například účet, fakturace, zabezpečení, smluvní evidence nebo analytika na základě souhlasu), je Václav Loubek správcem. Pokud škola nebo jiná organizace používá Team / School / Campus a prostřednictvím Syllonautu zpracovává údaje studentů pro vlastní výuku, je tato organizace zpravidla správcem a Syllonaut zpracovatelem podle samostatné zpracovatelské smlouvy (DPA).', 'For data Syllonaut processes for its own purposes (for example account administration, billing, security, contractual evidence or consent-based analytics), Václav Loubek is the controller. Where a school or other organisation uses Team / School / Campus to process student data for its own teaching purposes, that organisation is generally the controller and Syllonaut is its processor under a separate Data Processing Addendum (DPA).')}</p>
        </section>
        ...
        <section>
          ...
          <ul>
            ...
            <li><strong>{ui('Přijetí DPA organizací:', 'Organisation DPA acceptance:')}</strong> {ui('u objednávek Team / School / Campus evidujeme technický identifikátor objednávajícího účtu, verzi a acceptance key DPA a serverový čas přijetí v billing snapshotu objednávky.', 'for Team / School / Campus orders we record the ordering account technical identifier, DPA version and acceptance key, and the server acceptance time in the order billing snapshot.')}</li>
        ...
        <section>
          ...
          <p>{ui('Pokud Syllonaut zpracovává osobní údaje výhradně jménem školní nebo jiné organizace jako její zpracovatel, právní titul tohoto zpracování určuje daná organizace jako správce. Syllonaut v tomto rozsahu jedná podle jejích doložených pokynů a zpracovatelská smlouva podle čl. 28 GDPR je dostupná na stránce DPA.', 'Where Syllonaut processes personal data solely on behalf of a school or other organisation as its processor, the legal basis for that processing is determined by that organisation as controller. In that scope Syllonaut acts on its documented instructions and the Article 28 GDPR Data Processing Addendum is available on the DPA page.')}</p>
        </section>
        ...
        <section>
          ...
          <p>{ui('U tarifů Team / School / Campus je škola nebo jiná organizace správcem osobních údajů studentů a dalšího obsahu, který prostřednictvím Syllonautu zpracovává pro vlastní výukové účely; Syllonaut je v tomto rozsahu jejím zpracovatelem. Tento vztah upravuje ', 'For Team / School / Campus plans, the school or other organisation is the controller of student personal data and other content it processes through Syllonaut for its own teaching purposes; Syllonaut acts as its processor in that scope. This relationship is governed by the ')}<Link href={`/${locale}/dpa`}>{ui('zpracovatelská smlouva (DPA)', 'Data Processing Addendum (DPA)')}</Link>{ui(' podle čl. 28 GDPR. DPA se přijímá výslovně při objednávce organizace a její verze se eviduje v objednávkovém snapshotu.', ' under Article 28 GDPR. The DPA is expressly accepted during the organisation order and its version is recorded in the order snapshot.')}</p>
        </section>

        <section>
          <h2>{ui('8. Dodavatelé', '8. Service providers')}</h2>
          <p>{ui('Pokud dodavatel zpracovává údaje organizace jménem Syllonautu v jeho roli zpracovatele, jde o subprocesora a vztahuje se na něj režim uvedený v ', 'Where a provider processes organisation data on Syllonaut’s behalf in its processor role, that provider is a sub-processor and is governed by the regime set out in the ')}<Link href={`/${locale}/dpa`}>DPA</Link>.</p>
          <ul>
        ...
        <section>
          ...
          <ul>
            ...
            <li>{ui('U osobních údajů zpracovávaných jménem organizace se po skončení služby uplatní pokyny správce a pravidla výmazu nebo vrácení podle DPA; zákonné záznamy, které Syllonaut uchovává pro vlastní účely jako správce, tím nejsou dotčeny.', 'For personal data processed on behalf of an organisation, the controller’s instructions and the DPA deletion/return rules apply after the service ends; statutory records Syllonaut retains for its own purposes as controller are unaffected.')}</li>
