    ...
    <main className={landing.page}>
      ...
      <article className={styles.page}>
        <div className={styles.hero}>
          ...
          <div className={styles.meta}>{ui('Verze 1.11 · účinná od 26. 9. 2026', 'Version 1.11 · effective from 26 September 2026')}</div>
        </div>
        ...
        <section>
          ...
          <ul>
            ...
            <li><strong>{ui('Potvrzení upozornění na jedno živé použití lekce ve Free:', 'Acknowledgement of the Free single live use notice:')}</strong> {ui('když ve Free tarifu zavřete upozornění, že každou lekci lze živě použít jednou (tlačítkem „OK, rozumím“ nebo křížkem), evidujeme technický identifikátor účtu, lekci, u které jste upozornění zavřeli, verzi a přesný text upozornění, jazyk, způsob zavření a serverový čas. E-mail ani jiný přímý identifikátor do záznamu nekopírujeme.', 'when you close the Free plan notice that each lesson can be used live once (with “OK, got it” or the close button), we record the technical account identifier, the lesson where you closed it, the version and exact text of the notice, the language, how it was closed and the server time. The email address or another direct identifier is not duplicated in the record.')}</li>
        ...
        <section>
          ...
          <div className={styles.tableWrap}><table><thead><tr><th>{ui('Účel', 'Purpose')}</th><th>{ui('Právní základ', 'Legal basis')}</th></tr></thead><tbody>
            ...
            <tr><td>{ui('Evidence, že jste byli ve Free tarifu upozorněni na jedno živé použití lekce, a ochrana právních nároků (např. při reklamaci)', 'Recording that you were informed about the single live use of a lesson on the Free plan and protecting legal claims (for example in a complaint)')}</td><td>{ui('Oprávněný zájem na doložení poskytnutých informací a na určení, výkonu či obhajobě právních nároků (čl. 6 odst. 1 písm. f GDPR)', 'Legitimate interest in evidencing the information provided and in establishing, exercising or defending legal claims (Art. 6(1)(f) GDPR)')}</td></tr>
        ...
        <section>
          ...
          <div className={styles.tableWrap}><table><thead><tr><th>{ui('Technologie', 'Technology')}</th><th>{ui('Účel', 'Purpose')}</th><th>{ui('Doba', 'Duration')}</th></tr></thead><tbody>
            ...
            <tr><td>{ui('Úložiště prohlížeče: localStorage', 'Browser storage: localStorage')} (<code>syllonaut-pending-email-verification-v1</code>)</td><td>{ui('E-mailová adresa nového účtu, který čeká na potvrzení kódem z e-mailu, aby se formulář pro zadání kódu znovu otevřel i po obnovení nebo zavření stránky; nezbytné. Ukládá se jen ve vašem prohlížeči a po potvrzení e-mailu nebo přihlášení se smaže.', 'The email address of a new account waiting to be confirmed with the emailed code, so the code form reopens even after the page is reloaded or closed; essential. It is stored only in your browser and deleted once the email is confirmed or you sign in.')}</td><td>{ui('Nejvýše 24 hodin.', 'Up to 24 hours.')}</td></tr>
        ...
        <section>
          ...
          <ul>
            ...
            <li>{ui('Záznam o potvrzení upozornění na jedno živé použití lekce ve Free (ID účtu, lekce, verze a text upozornění, jazyk, způsob zavření a serverový čas) můžeme uchovat i po zrušení účtu po dobu přiměřeně nutnou k vyřízení reklamací a k určení, výkonu či obhajobě právních nároků; poté jej smažeme nebo anonymizujeme.', 'A record of acknowledging the Free single live use notice (account ID, lesson, notice version and text, language, how it was closed and server timestamp) may be retained after account deletion for as long as reasonably necessary to handle complaints and to establish, exercise or defend legal claims; it is then deleted or anonymised.')}</li>
