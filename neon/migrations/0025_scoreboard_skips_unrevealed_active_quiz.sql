-- The student scoreboard no longer counts the active quiz before its results
-- are revealed (2026-09-26).
--
-- With the scoreboard revealed, get_student_public_scoreboard counted every
-- quiz up to the active block. A student could answer the active quiz, watch
-- their score (it refreshes about 3 s after the answer) and, while the block
-- is active and its results are not revealed, change the answer until the
-- score went up.
--
-- The only change against the production definition (0012 patched in place,
-- md5 809ca2e910f505b46b10bd467a012abe on 2026-09-26): the session's
-- revealed_block_ids are read, and while the session is live, the active quiz
-- is left out of scored_blocks until its block id is in revealed_block_ids.
-- It therefore counts toward neither the score, maxPoints nor the rank.
-- Earlier quizzes still count: the student cannot change those answers once
-- the teacher moves on. Signature, SECURITY DEFINER, search_path and grants
-- are unchanged (CREATE OR REPLACE keeps the grants). Idempotent.
...
begin
  ...
  select p.id, s.status, s.active_block_id, s.lesson_snapshot, s.scoreboard_revealed, s.revealed_block_ids
  into v_participant_id, v_status, v_active_block_id, v_lesson_snapshot, v_scoreboard_revealed, v_revealed_block_ids
  ...
  scored_blocks as (
    ...
    -- The active quiz counts only after its results are revealed; before that
    -- the score would tell the student whether a still changeable answer is correct.
    and not (
      v_status = 'live'
