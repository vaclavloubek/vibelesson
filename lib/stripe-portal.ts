...
export async function createStripeSandboxPortalSession(input: {
  ...
  if (!/^cus_[A-Za-z0-9_]+$/.test(input.customerId)) {
    throw new Error('stripe_customer_id_invalid');
  }

  const params = new URLSearchParams();
  params.set('customer', input.customerId);
  params.set('return_url', 'https://www.syllonaut.com/pricing?portal=returned');
  ...
  if (!response.ok) {
    ...
    throw new StripePortalApiError(
  ...
  if (!payload.id?.startsWith('bps_') || !payload.url?.startsWith('https://billing.stripe.com/')) {
