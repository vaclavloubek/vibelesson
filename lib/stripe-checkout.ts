...
export function buildStripeCheckoutParams(input: Omit<CreateStripeCheckoutInput, 'secretKey'>) {
  ...
  params.set('metadata[syllonaut_early_performance_requested]', input.earlyPerformanceRequested ? 'true' : 'false');
  ...
  params.set('subscription_data[metadata][syllonaut_early_performance_requested]', input.earlyPerformanceRequested ? 'true' : 'false');
