...
async function verifyParticipant(sessionId: string, participantToken: string) {
  ...
  const { data, error } = await db
    ...
    .eq('participant_token_hash', hash(participantToken))
...
async function loadContext(body: Record<string, unknown>, requireActive = true) {
  ...
  const [{ data: session, error: sessionError }, { data: team, error: teamError }] = await Promise.all([
    db.from('sessions').select('status, active_block_id, lesson_snapshot, realtime_key').eq('id', sessionId).maybeSingle(),
    db.from('teams').select('id').eq('id', participant.team_id).eq('session_id', sessionId).maybeSingle(),
  ]);
  ...
  return {
    context: { participant, session: session as SessionRow, blockId } as Context,
    sessionId,
  };
}
...
async function lockInfo(sessionId: string, teamId: string, blockId: string, viewerParticipantId: string) {
  ...
  return {
    ...
    holderDisplayName: holder?.display_name ? String(holder.display_name) : 'Člen týmu',
...
async function claimLock(context: Context, sessionId: string) {
  const db = createAdminClient();
  const { data, error } = await db.rpc('claim_team_edit_lock', {
...
async function save(body: Record<string, unknown>) {
  ...
  const { data: saved, error } = await db
    ...
    .single();
  if (error || !saved) {
    console.error('Autosave team response failed', error);
...
async function submit(body: Record<string, unknown>) {
  ...
  const { data: saved, error } = await db
    ...
    .single();
  if (error || !saved) {
    console.error('Submit team response save failed', error);
    return freeSessionError(error) ?? json({ error: 'Týmovou odpověď se nepodařilo odevzdat.' }, 500);
  }
  const { data: queued, error: queueError } = await db.rpc('queue_submitted_team_response_evaluation', {
...
export async function handleTeamEditAction(body: Record<string, unknown>): Promise<Response> {
  ...
  try {
    ...
    console.error('team-edit server module failed', error);
