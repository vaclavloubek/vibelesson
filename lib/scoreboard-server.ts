...
export type ScoreSource = 'quiz' | 'ai' | 'teacher' | 'pending' | 'failed' | 'missing';
...
export type TeacherScoreboardData = {
...
export type TeacherScoreboardLoadResult =
  | { data: TeacherScoreboardData }
...
export async function loadTeacherScoreboard(
  sessionId: string,
  userId: string,
  ...
  if (sessionError) {
    ...
    return { error: 'Hodinu se nepodařilo načíst.', status: 500 };
  }
  if (!session) return { error: 'Hodina nebyla nalezena.', status: 404 };

  const lesson = LessonSchema.safeParse(session.lesson_snapshot);
  if (!lesson.success) return { error: 'Snapshot lekce je neplatný.', status: 500 };
  ...
  if (participantsResult.error || responsesResult.error || evaluationsResult.error) {
    ...
    return { error: 'Průběžné skóre se nepodařilo načíst.', status: 500 };
  }
  ...
  return {
    data: {
      status: session.status as 'lobby' | 'live' | 'ended',
      title: lesson.data.title,
      realtimeKey: session.realtime_key as string,
